...
# then verify the alert left the active queue after its short endsAt.
#
...
#
# Alertmanager rejects a POST whose endsAt is in the past ("start time must
# be before end time"), so resolve is expressed as a short endsAt: the probe
# posts with endsAt=+EXPIRE_SECONDS and waits it out — a crashed probe can
# never leave a firing alert, and the resolve path (group exit, resolved
# notifications) is exercised exactly like a real recovery. Delivery failure
# is also detected continuously by the AlertDeliveryFailed rule
...
EXPIRE_SECONDS="${SYNTHETIC_EXPIRE_SECONDS:-120}"
...
post_alert() {
  curl -sf -X POST "$ALERTMANAGER_URL/api/v2/alerts" \
    ...
    -d "[{\"labels\":{\"alertname\":\"$ALERTNAME\",\"severity\":\"$SEVERITY\",\"job\":\"synthetic\",\"probe\":\"routing\"},\"annotations\":{\"summary\":\"Synthetic $SEVERITY routing check\",\"description\":\"Scheduled end-to-end alert-path probe (#683). Safe to ignore.\"},\"startsAt\":\"$(date -u +%Y-%m-%dT%H:%M:%S.000Z)\",\"endsAt\":\"$(ends_at "$EXPIRE_SECONDS")\"}]" \
    >/dev/null
}
...
echo "[synthetic] firing $ALERTNAME severity=$SEVERITY (endsAt=+${EXPIRE_SECONDS}s) at $(date -Is)"
if ! post_alert; then
...
if [ "$PENDING" -eq 0 ]; then
  echo "ERROR: synthetic alert vanished before delivery — check Alertmanager logs" >&2
  exit 1
fi

# Wait out the endsAt so the group resolves (resolved notifications fire on
# every receiver), then confirm the alert left the active queue.
sleep "$EXPIRE_SECONDS"

REMAINING=$(alerts_pending)
if [ "$REMAINING" -ne 0 ]; then
  echo "ERROR: synthetic alert did not resolve cleanly after endsAt" >&2
  exit 1
fi

echo "[synthetic] OK: $ALERTNAME ($SEVERITY) fired, fanned out, and resolved through the full path"
