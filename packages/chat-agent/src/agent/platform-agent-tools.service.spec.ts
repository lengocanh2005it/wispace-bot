...
describe('PlatformAgentToolsService', () => {
  ...
  describe('zalo behavior', () => {
    ...
    it('calls the Wispace API with the WISPACE userId (zalo historical behavior)', async () => {
      ...
      expect(goalsSpy).toHaveBeenCalledWith('42', { signal: undefined });
    });
  ...
  describe('toolOverrides (messenger-style)', () => {
    beforeEach(() => {
      service = new PlatformAgentToolsService(
        goalsService as unknown as WispaceGoalsService,
        calendarService as unknown as WispaceCalendarService,
    ...
    it('dispatches overridden tools to the platform implementation', async () => {
      const override = jest.fn().mockResolvedValue({ report: 'LLM report' });
      service = new PlatformAgentToolsService(
        goalsService as unknown as WispaceGoalsService,
        calendarService as unknown as WispaceCalendarService,
        stagePort,
        {
          ...buildDiscordOptions(confirmSender),
          toolOverrides: { get_learning_progress_report: override },
        },
      );
    ...
    it('falls back to the shared implementation for non-overridden tools', async () => {
      goalsService.getUserGoals.mockResolvedValue({ targetBand: '7.0' });
      service = new PlatformAgentToolsService(
        goalsService as unknown as WispaceGoalsService,
        calendarService as unknown as WispaceCalendarService,
        stagePort,
        {
          ...buildDiscordOptions(confirmSender),
          toolOverrides: { get_learning_progress_report: jest.fn() },
        },
      );
    ...
    it('works when the shared Wispace services are not provided', async () => {
      const override = jest.fn().mockResolvedValue({ ok: true });
      const noWispace = new PlatformAgentToolsService(
        undefined,
        undefined,
        stagePort,
        {
          ...buildDiscordOptions(confirmSender),
          toolOverrides: { get_user_goals: override },
        },
      );

      const result = await noWispace.execute('get_user_goals', '{}', {
        externalUserId: 'psid-1',
        userId: 42,
      });
