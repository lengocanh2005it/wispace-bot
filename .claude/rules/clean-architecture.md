...
- **Do not** import NestJS, TypeORM, or Redis in `@wispace/chat-metering/core` — explicit module, entity, repository, config, and platform adapters are exposed from `/adapters`. Core policies consume ports (including the safety repository port) and the provider-neutral `LlmUsage` contract; no OpenAI SDK types enter the metering core (#427).
