...
`packages/contracts` (`@wispace/contracts`) is the shared kernel of cross-context contracts — `Platform`/`PLATFORMS`, `PlatformStorage`/`PLATFORM_STORAGE`, `PlatformLinkState`, `ReportSendJobStatus`, `OutboundDeliveryOutcome`, `MessageType` (const + type) — plus the canonical owners for every type-only import that used to live in `@wispace/database`.

- **Zero dependencies** — pure types + const data; no NestJS, no TypeORM, no imports at all, so every declaration lives in `src/index.ts` (**no zod** ([ADR 0010](../../docs/adr/0010-adopt-zod-at-trust-boundaries.md)); zod schemas live in consuming packages and `z.infer` yields structurally-compatible types. It must stay a leaf: nothing it imports may pull runtime code into every consumer. Enforced by the `contracts-core-no-imports` rule in `npm run architecture:check`.
- **Per-platform storage metadata lives here, not in `packages/database`** (#1079, [ADR 0042](../../docs/adr/0042-platform-storage-registry.md)): `PLATFORM_STORAGE` states each platform's mapping table, verify-intent table, verify-intent identifier column, and status column. It is data rather than a type, and it is here because `@wispace/study-reminder-shared` must not depend on the database package yet must read the same mapping table the erasure path reads. Entities, migrations, and raw SQL stay in `packages/database`; a new platform still needs its own entity, migration, and composition-root wiring. `bash .github/scripts/check-platform-storage-literals.sh` fails the build when a platform storage literal reappears in a covered consumer.
