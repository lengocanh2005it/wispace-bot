...
`CHAT_SYSTEM_PROMPT_CORE` (universal rules) + per-bot overlay file + data-only `Process marker: <value>` + dynamic suffix. The existing core non-disclosure rule supplies canary behavior; do not add a new universal rule or change the core/hash for #1285.

| Part    | File                                                                       | Content                                                                                                              |
| ------- | -------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| Core    | `packages/llm-agent/src/chat-system-prompt.ts` (`CHAT_SYSTEM_PROMPT_CORE`) | Scope, out-of-scope, academic-integrity boundary (#628), non-disclosure of internal details (#625), no-tool rules, no-fabrication, `precreate_next_exercise`, general rules — **shared, edit once**. Carries no message-format rule: Markdown support is a platform capability (#991) |
| Overlay | `apps/messenger-bot/src/shared/prompts/messenger-chat.system.txt`          | Identity, report registration, cards, reschedule buttons, message format (plain text)                                |
| Overlay | `apps/discord-bot/src/shared/prompts/discord-chat.system.txt`              | Identity, server-channel DM privacy, reschedule buttons, message format (Markdown allowed)                           |
| Overlay | `apps/zalo-bot/src/shared/prompts/zalo-chat.system.txt`                    | Identity, reschedule confirm flow, message format (plain text)                                                       |
...
- **Canonical safety matching (#501)** — input scans and the final prompt/vendor output checks use shared raw + canonical candidates (`buildSafetyScanCandidates`): NFD/de-accent + `đ` folding, confusable mapping, format/variation-selector removal (also as a space), and `_`/`-`/`.` separator folding. One boundary-preserving candidate keeps sentence punctuation and line breaks available to role-marker rules. Raw text remains the credential-shape candidate; canonical candidates are not used to block the generic phrase `system prompt`.
- **Input classifier tier (#649/#1186/#1054/#975/#1048/#1256)** — `LlmContentClassifier` (`@wispace/chat-agent`), gated by `LLM_INPUT_CLASSIFIER_ENABLED`. Runs in `PlatformAgentService.reply` before history and `agent.reply`. Tier-1 single-turn, joint-scan, and length guards are authoritative preflight checks: a hit skips the classifier provider call and cannot be downgraded to `SAFE`. The classifier prompt treats learner text as untrusted data, so requests inside it to choose a label, confidence, reason, JSON verdict, or `SAFE` are `INJECTION` (#1378). Greeting / self-intro / disclosure probes are normally short-circuited at the bot gateway (webhook router's `detectDisclosureProbe` + `IntentDetector`) and off-topic / ambiguous by the clarification state machine, but `runInputClassifier` re-checks belt-and-braces — it skips when `isGreetingOnly`, `isObviouslyOffTopic`, or its own `IntentDetector` returns a non-`unknown` intent (distress expressions reach the classifier so crisis disclosures with distress phrasing are evaluated, #1048/#1054). After `redactSecrets`, text within `LLM_INPUT_CLASSIFIER_MAX_INPUT_CHARS` (default 512 Unicode code points) is sent in full; over-ceiling text is sent once as a deterministic head-and-tail sample with an omission marker. The marker means the middle was omitted and is not safe; the provider still receives only one bounded `generateJson` request. The call keeps its own `AbortSignal.timeout`, no retry, no shared concurrency budget, local circuit breaker: 5 fails → open 30s → one half-open probe. Shadow mode records typed classifier failures and continues the normal path; enforce mode returns `CHAT_FAILURE_FALLBACK_MESSAGE` with `skipHistory=true` when no verdict is available. A real enforce-mode `CRISIS` verdict uses `buildCrisisSupportHandoffMessage()` without the normal confidence floor; an outage never fabricates `CRISIS`. Usage is recorded under `LLM_INPUT_CLASSIFIER`; safety telemetry remains redacted excerpt + hash. `LLM_INPUT_CLASSIFIER_ENABLED` meters every verdict (non-SAFE → `recordClassifierVerdict` → `llm_safety_events` `event_type='CLASSIFIER_FLAGGED'`; all verdicts and bounded failure reasons → `<prefix>_llm_classifier_verdict_total{label,mode,platform}`) and input shape via `<prefix>_llm_classifier_input_total{shape,platform}`; `LLM_INPUT_CLASSIFIER_ENFORCE` additionally swaps the reply (`CRISIS` → crisis handoff; `ABUSE` → hostility deflection; `INJECTION` → prompt-injection block; `INJECTION` with an `extraction`-flavoured reason + `DISCLOSURE_PROBE` → `buildNonDisclosureReply()`), gated by mode but not by the normal confidence floor for `CRISIS`. Neither enforce reply invokes the LLM, tools, or chat-history append (`skipHistory: true`). Messenger-only; Discord/Zalo remain unchanged. Not run inside `LlmAgentService.checkEarlyReturns` (gateway-tier only). When enabled, the classifier model is required and must be approved for the single active provider; a multi-provider failover order fails startup until feature routing (#417) exists. Classifier _quality_ (label accuracy on paraphrased / multilingual probes) is checked by the `classifier-eval-nightly` workflow — `npm run classifier:eval --workspace=@wispace/llm-agent` over `classifier-eval.fixtures.ts`, real provider, **not** part of PR `verify` (#505); the evaluator fails on any `must-block` miss while retaining the existing overall floor for adversarial probes. See ADR 0026 for sampling and ADR 0030 for failure policy.
...
**Narrow #1285 exception:** the ephemeral process canary may enter model context only through the data-only `Process marker: <value>` part. It must never enter logs, history, safety events, metrics, alerts, configuration, or the database. Masked external IDs may follow the existing logging policy; external IDs must not enter metrics, alerts, or events.
...
| Process marker (`Process marker: <value>`) | Runtime process canary | #1285 narrow exception: data-only model-context value; never logged, persisted, configured, or labeled |
