...
- **Input classifier tier (#649)** — `LlmContentClassifier` (`@wispace/chat-agent`), gated by `LLM_INPUT_CLASSIFIER_ENABLED`. Runs in `PlatformAgentService.replyInternal` just before `agent.reply`. Greeting / self-intro / disclosure probes are normally short-circuited at the bot gateway (webhook router's `detectDisclosureProbe` + `IntentDetector`) and off-topic / ambiguous by the clarification state machine, but `runInputClassifier` re-checks belt-and-braces — it skips when `isDistressExpression`, `isGreetingOnly`, `isObviouslyOffTopic`, or its own `IntentDetector` returns a non-`unknown` intent. Labels `SAFE | INJECTION | DISCLOSURE_PROBE` via a cheap-model `generateJson` call on its own path (own `AbortSignal.timeout`, no retry, no shared concurrency budget, local circuit breaker: 5 fails → open 30s → one half-open probe), fail-open on any failure. Ships shadow-first: `LLM_INPUT_CLASSIFIER_ENABLED` meters every verdict (non-SAFE → `recordClassifierVerdict` → `llm_safety_events` `event_type='CLASSIFIER_FLAGGED'`; all verdicts → `<prefix>_llm_classifier_verdict_total{label,mode,platform}`), `LLM_INPUT_CLASSIFIER_ENFORCE` additionally swaps the reply (`INJECTION` → `buildPromptInjectionBlockedMessage()`; `INJECTION` with an `extraction`-flavoured reason + `DISCLOSURE_PROBE` → `buildNonDisclosureReply()`), gated by `LLM_INPUT_CLASSIFIER_MIN_CONFIDENCE`. Messenger-only during the shadow window (`AgentMetricsPort.classifierVerdictInc`; Discord/Zalo noop). Not run inside `LlmAgentService.checkEarlyReturns` (gateway-tier only). When enabled, the classifier model is required and must be approved for the single active provider; a multi-provider failover order fails startup until feature routing (#417) exists. Classifier _quality_ (label accuracy on paraphrased / multilingual probes) is checked by the `classifier-eval-nightly` workflow — `npm run classifier:eval --workspace=@wispace/llm-agent` over `classifier-eval.fixtures.ts`, real provider, **not** part of PR `verify` (#505).
