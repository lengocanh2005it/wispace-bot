...
- **Input classifier tier (#649/#1186/#1054/#975/#1048)** — `LlmContentClassifier` (`@wispace/chat-agent`), gated by `LLM_INPUT_CLASSIFIER_ENABLED`. Runs in `PlatformAgentService.reply` before history and `agent.reply`. Greeting / self-intro / disclosure probes are normally short-circuited at the bot gateway (webhook router's `detectDisclosureProbe` + `IntentDetector`) and off-topic / ambiguous by the clarification state machine, but `runInputClassifier` re-checks belt-and-braces — it skips when `isGreetingOnly`, `isObviouslyOffTopic`, or its own `IntentDetector` returns a non-`unknown` intent (distress expressions reach the classifier so crisis disclosures with distress phrasing are evaluated, #1048/#1054). After `redactSecrets`, text within `LLM_INPUT_CLASSIFIER_MAX_INPUT_CHARS` (default 512 Unicode code points) is sent in full; over-ceiling text is sent once as a deterministic head-and-tail sample with an omission marker. The marker means the middle was omitted and is not safe; the provider still receives only one bounded `generateJson` request. The call keeps its own `AbortSignal.timeout`, no retry, no shared concurrency budget, local circuit breaker: 5 fails → open 30s → one half-open probe, fail-open on any failure. Ships shadow-first: `LLM_INPUT_CLASSIFIER_ENABLED` meters every verdict (non-SAFE → `recordClassifierVerdict` → `llm_safety_events` `event_type='CLASSIFIER_FLAGGED'`; all verdicts → `<prefix>_llm_classifier_verdict_total{label,mode,platform}`) and input shape via `<prefix>_llm_classifier_input_total{shape,platform}`; `LLM_INPUT_CLASSIFIER_ENFORCE` additionally swaps the reply (`CRISIS` → `buildCrisisSupportHandoffMessage()`; `ABUSE` → `buildHostilityDeflectionMessage()`; `INJECTION` → `buildPromptInjectionBlockedMessage()`; `INJECTION` with an `extraction`-flavoured reason + `DISCLOSURE_PROBE` → `buildNonDisclosureReply()`), gated by `LLM_INPUT_CLASSIFIER_MIN_CONFIDENCE`. Neither enforce reply invokes the LLM, tools, or chat-history append (`skipHistory: true`). Messenger-only during the shadow window; Discord/Zalo remain unchanged. Not run inside `LlmAgentService.checkEarlyReturns` (gateway-tier only). When enabled, the classifier model is required and must be approved for the single active provider; a multi-provider failover order fails startup until feature routing (#417) exists. Classifier _quality_ (label accuracy on paraphrased / multilingual probes) is checked by the `classifier-eval-nightly` workflow — `npm run classifier:eval --workspace=@wispace/llm-agent` over `classifier-eval.fixtures.ts`, real provider, **not** part of PR `verify` (#505). See ADR 0026 for the accepted sampling trade-off.
