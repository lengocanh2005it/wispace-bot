...
- `messenger_link_verify_records` — one durable Messenger verify intent per PSID; stores only a ref fingerprint, metadata, generation, and pending/processing/committed state plus a short owner lease; persistence reserves the lease for the verifying callback, mapping confirms the exact lease before side effects, in-flight generations are not superseded, expired claims are reconciled, and rows are erased by privacy deletion.
- `discord_link_verify_records`, `zalo_link_verify_records` — durable platform verify intents; reconciled by their five-minute account-link cron after a callback crash. Each row carries an intent generation plus the pre-verify mapping observation (`absent` or mapping generation) so callback/reconcile upserts are compare-and-set fenced; migration `1789093200000` retires legacy rows that cannot be fenced.
...
From the repository root, `npm run database:entity-discovery:test` verifies that compiled Discord/Zalo entity modules are discovered from `dist` without including transformer/helper/test artifacts, and that every discovered entity is registered by the production TypeORM options. Run it after building the bot packages. `npm run database:bootstrap-smoke` then validates the discovered Discord/Zalo entity metadata and OAuth-state cleanup against a disposable PostgreSQL database using `synchronize` (migrations intentionally excluded). `npm run database:query-returning-shape-smoke` pins the raw `query()` result shapes TypeORM's Postgres runner produces — `UPDATE`/`DELETE` (even zero-match) return the `[rows, rowCount]` tuple, `INSERT ... ON CONFLICT DO UPDATE` stays flat — the contract the shared `extractQueryRows` helper in `@wispace/bot-common/utils` depends on (#754); any raw `UPDATE`/`DELETE ... RETURNING` site must consume rows through that helper, never a hand-rolled `Array.isArray` check. `npm run database:privacy-smoke` resets a disposable loopback database and verifies real Postgres unlink/delete/export outcomes across Messenger, Discord, and Zalo while preserving message logs. It builds no registry of its own: it calls each app's exported `buildPrivacyEntityRegistry()` from `dist`, asserts the three agree on every shared target (only `platform` and `messageLog` may differ), and seeds the non-current platforms with **distinct** external ids so the cross-platform fan-out is asserted by its own audit rows rather than being indistinguishable from the current-platform path (#461). When `REDIS_HOST` is also set (loopback-only; CI always sets it), it additionally seeds real chat history, a real queue buffer, and a display-name cache entry through the same production store classes the apps use, then asserts `delete()` erases them — and, critically, seeds a **second, untouched identity** on the same platform and asserts its queue-buffer key _and_ its active-set membership survive, which is the regression check for a bulk-`DEL`-of-the-shared-set defect (#537) that a plain "key is gone" assertion would not catch. `npm run database:migration-compatibility` separately runs the canonical Messenger migration chain, verifies no pending migrations, and checks tables for the same discovered entities. `npm run database:persistence-semantics-smoke` pins the SQL/Lua semantics mocks cannot prove (#538/#849): cron-leader-lease fencing (acquire/expiry/takeover/fail-open/two-worker race), scheduled-report claim fencing on both paths (`PlatformReportClaimRepository` duplicate-claim race, stale-token no-ops after expired-lease reclaim, sent-not-reclaimable / released-reclaimable), the study-reminder two-worker `claimJob` race (fencing sequence itself lives in `study-reminder:delivery-smoke`), inbox `listDue`/`countDue` platform isolation, quota idempotency `ON CONFLICT` behavior, Lua burst/slot negatives on real Redis, and privacy-delete atomicity via a mid-transaction fault trigger. All database smoke commands require `NODE_ENV=test` and a loopback `DB_HOST`; never run them against `ai_chat_bot_db` production.
...
| Messenger linking                   | `1786940200000-HardenMessengerLinkVerifyRecords` + `1789093400000-HardenMessengerLinkVerifyLeases` | `messenger_link_verify_records` — generation-fenced replay, topic/cadence metadata, persistence-reserved lease confirmed before side effects, bounded stale reconciliation, committed cleanup |
...
- **Per-platform storage metadata is owned by `PLATFORM_STORAGE`** (#1079, ADR-0042) in `@wispace/contracts`: mapping table, mapping primary-key type, verify-intent table, verify-intent identifier column, and status column per platform. Never write a `Record<Platform, …>` of table or column names, and never test `platform === 'messenger'` to choose one — read the descriptor. It lives in the contracts package rather than here because `@wispace/study-reminder-shared` must not depend on this one yet must read the same mapping table the erasure path reads. `.github/scripts/check-platform-storage-literals.sh` (CI `deploy-scripts-test`) fails the build when a platform storage literal reappears in a covered consumer. Redis key prefixes, privacy cleanup store sets, canonical platform priority order, and advisory lock ids are **out of scope** and stay in their own registries. Entity `@Entity()` names, migration SQL, and the literal `UNION ALL` in `canonical-platform.service.ts` / `learner-usage-query.ts` are not covered by the guard and are left as declared.
