...
describe('DiscordStudentReportService', () => {
  ...
  it('creates StudentReportCore lazily on first generateReport call', async () => {
    ...
    await service.generateReport('discord-1');
  ...
  it('generates report with correlationId containing userId and date', async () => {
    ...
    expect(mockGenerateReport).toHaveBeenCalledWith('discord-1', {
      // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
      correlationId: expect.stringContaining('discord-1:'),
    });
  });
  ...
  it('returns the report text from the core', async () => {
    ...
    const result: string = await service.generateReport('discord-1');
