...
- Production env sync: use the manual `sync-env.yml` workflow to refresh the per-bot Vault AppRole bootstrap; production bundles contain no full secret env and the in-container Doppler webhook is disabled because bot containers do not receive the host Docker socket. Legacy Doppler cleanup is tracked in #655.
...
| Vault runtime secret contract (#653/#654)                                           | Shared/per-bot KV v2 bootstrap plus Vault-only production delivery — [vault-secrets.md](docs/vault-secrets.md); legacy Doppler removal remains #655                                      |
