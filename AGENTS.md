...
Instructions for AI coding agents working in the **wispace-bots** repo — Turborepo monorepo for WISPACE student bots (AI reports + study reminders + rate-limited AI chat). Currently includes `apps/messenger-bot` (fully functional), `apps/discord-bot` (fully functional: chat + quota/usage/safety + account-linking OAuth2 + 7/7 real tool handlers + 08:00 report cron + leader-election + retry dispatch + study reminders + dead letter + message log cleanup + CI/CD workflow), `apps/zalo-bot` (fully functional: chat + quota + account-linking OAuth2 + 7/7 real tool handlers + LLM report enrichment + 08:00 report cron + study reminders + dead letter + message log + stuck recovery + ops endpoints + CI/CD + shared health endpoints + Redis queue/history options + chat queue; production env sync uses the manual Vault workflow), and 21 shared packages in `packages/`: `contracts` (shared kernel contracts — `Platform`, `PlatformLinkState`, `ReportSendJobStatus`, `OutboundDeliveryOutcome`, `MessageType`; the canonical owner of cross-context contracts — type-only imports of `@wispace/database` are banned outside it by `.github/scripts/check-database-type-imports.sh`, CI job `deploy-scripts-test`; chat-quota contracts live in `chat-metering`, `StudyReminderJobStatus` in `study-reminder-shared`, persistence-only states stay local to `database`), `llm-agent` (LLM function-calling + provider abstraction), `chat-metering` (quota/rate-limit + LLM usage/safety event tracking), `chat-agent` (platform-parameterized agent/tools/queue/history services for Discord & Zalo), `wispace-client` (Wispace API HTTP client for goals/scores/calendar), `chat-history` (memory + Redis chat history stores with TTL + turn cap), `student-report` (student capability report generation), `chat-queue-core` (per-user debounce/merge state machine), `chat-pipeline` (platform-agnostic chat pipeline: rate-limit → history → agent → outbound), `learner-profile` (compact per-learner facts — band target, exam date — sourced only from server-derived tool results, injected into chat prompts with 24h freshness rules; `get_user_goals` is the v1 facts source), `study-reminder-shared` (study reminder schedule + dispatch/sync/worker services), `scheduler-core` (report cron scheduling + leader election), `account-link-core` (framework-free completion/reconcile/OAuth-state policies for Discord and Zalo), `bot-metrics` (Prometheus metrics), `cleanup-cron` (advisory-lock cleanup cron), `ops-health` (ops health snapshot + alerts), `reschedule-confirm` (generic reschedule confirmation service), `bot-common` (shared NestJS infrastructure: ops API guard, advisory locks), `database` (shared DB entities + migrations), `webhook-inbound` (durable webhook inbound inbox — ingest/retry-cron/retention cleanup extracted from `database` by #426; entities/migrations/types stay in `database`, apps must not import the workflow via `@wispace/database` — CI check `.github/scripts/check-webhook-inbound-database-imports.sh`), `date-utils` (timezone-aware date helpers).
...
- Copy `.env.example` → `.env` and fill in real tokens before running sync/cron. Production uses the [Vault bootstrap runbook](docs/vault-secrets.md).
...
- **PgBouncer (production, optional):** `deploy/docker-compose.pgbouncer.yml` uses the pinned `edoburu/pgbouncer` image and generates auth files from `PGBOUNCER_DB_USER`/`PGBOUNCER_DB_PASSWORD`; it joins both `app_n8n_db_network` and `monitoring`. `PGBOUNCER_DB_HOST` must point at the managed PostgreSQL writer endpoint (no silent single-host fallback). Bots use `DB_HOST=pgbouncer`, `DB_PORT=5432`, and `DB_ALLOW_INSECURE_HOSTS=pgbouncer`; `vps-deploy.sh` attaches bot containers to `app_n8n_db_network` as well as `monitoring`. Keep `pool_mode=session` because advisory locks require one backend connection per client session; do not add unsupported `auth_password` to `pgbouncer.ini`.
...
- **VPS self-pull deploy** (#144/#172/#275/#283/#278/#408): `.github/scripts/vps-self-pull-deploy.sh` runs `git fetch`/`reset` **inside** the deploy lock (cron line is just `bash .github/scripts/vps-self-pull-deploy.sh` — fetch/reset must NOT move back into the crontab line). Fetch/reset/stale-checkout failures fail closed: timestamped `ERROR` in `~/vps-self-pull-deploy.log`, stall marker in `~/.vps-deploy-state/stall`, and a Telegram alert via the local Alertmanager (`vps_self_pull_stall`, default route) — resolved alert posted on the next healthy tick. **Per-app deploy failures** (#202) alert once per `(app, sha)` via `~/.vps-deploy-state/<app>.failed` (`vps_self_pull_app_failed`), cleared+resolved on a later success. **Deploy hardening** (#199/#201/#203/#204/#275/#278/#408): `vps-deploy.sh` fails closed on missing `.env` / migration writer endpoint / upstream conf (`SKIP_NGINX_CHECK=true` = first-deploy escape); the release image's `migration:show` must report an applied migration set with no pending entries before nginx cutover; live container detected by nginx-routed port (interrupted blue-green adopts `${APP}-new` as `-old`, never deletes it); `DOCKER_STOP_TIMEOUT` (60s) covers the 45s drain; the TypeORM migration data source verifies the writer and holds `MIGRATION_LOCK_ID` on a dedicated session before invoking the migration executor; post-switch monitor verifies the public nginx route (`/health[/discord|/zalo]/ready` via `curl --resolve`); uploads exclude `.env` from `rsync --delete`; env files `mktemp`+`chmod 600`+EXIT trap; `postgres-backup.sh` umask 077 + 700/600. **Bootstrap rollback (#932):** the deploy logs in with the bootstrap AppRole against Vault **before** installing it (rejected/expired credential → deploy refuses, live `.env` untouched); a snapshot `.env.pre-deploy` (0600) is taken before the atomic install and the EXIT trap restores it byte-identically on any failed/aborted exit after the install (first deploy removes the installed `.env` instead) — a rolled-back deploy no longer leaves a bad bootstrap armed for the next container restart; success drops the snapshot. **Metrics recovery (#278):** bare blue-green containers join the external `monitoring` network, Prometheus uses stable `<app>-metrics` aliases on fixed internal ports, and deploy verifies protected `/metrics` before/after alias handoff with rollback on failure. **Migration barrier** (#283): self-pull deploys Messenger first; Discord/Zalo are skipped until Messenger's migration-owner deploy succeeds or is already at the target SHA. **Supply chain** (#193/#196): Turbo secrets (`TURBO_TOKEN`/`TURBO_TEAM`) are scoped to the `Verify` step only (not exposed during `npm ci`); deploy scripts pin images by immutable digest extracted from `docker manifest inspect` — `docker pull/run` use `@sha256:<digest>` when available, fail closed if digest can't be verified. Recovery + setup: `docs/project-overview.md` §12. Script behavior is regression-tested by `.github/scripts/tests/vps-self-pull-deploy.test.sh` + `.github/scripts/tests/vps-deploy.test.sh` (CI job `deploy-scripts-test`) — update them when touching the scripts.
- **Postgres offsite recovery (#866/#879):** the Messenger deploy workflow reads the shared Vault path and installs a separate mode-600 host-only `backup-bootstrap.env` at `/home/ngoc_anh/backups/ai_chat_bot_db/backup.env`; it is never passed to bot containers. `postgres-backup.sh` and `postgres-offsite-sync.sh` use that file; offsite sync promotes checksum-verified encrypted dump/globals/state-sidecar/pre-migration/evidence plus a manifest through rclone and never deletes remote objects. `backup-monitor.sh` checks the remote manifest/object sidecars hourly (manifest v2 — a v1 set without the state sidecar fails closed) and downloads the remote dump+globals+state pair for guarded restore verification weekly. The restore verifier (`deploy/postgres-restore-verify.sh`, #879) requires `--state-artifact` (backup-time migration/table inventory, captured before the dump), proves data integrity by subset-checking restored migrations/tables against the sidecar (empty/partial/stale restore fails; extra entries warn) plus non-PII row floors (`RESTORE_VERIFY_MIN_ROWS`, default `user_platform_mappings:1`); staging targets require TLS (`RESTORE_TARGET_CA`, `sslmode=verify-full`), a fresh/empty DB, rejection when the staging host resolves to a production IP (`getent ahostsv4`), and receive sanitized globals (allowlisted roles only, no passwords/SUPERUSER/ACLs); evidence is collision-resistant (`restore-verify-<ts>-<hex>.json`) and pruned after `RESTORE_VERIFY_EVIDENCE_RETENTION_DAYS` (default 30). One retained staging-drill evidence run is the DR proof — the next offsite sync uploads it permanently. Bucket versioning/Object Lock and lifecycle retention are operator prerequisites.
...
- Ops HTTP (`/messenger/study-calendar/sync`, `send-reports`, `/messenger/ops/clarification/clear`, …) require header **`X-Internal-Api-Key`** or `Authorization: Bearer …` matching `INTERNAL_API_KEY`.
- **Re-engagement (#850/#853):** module `discord-reengagement` — `POST /v1/discord/reengagement/run-once` (ops-guarded, `{userId}`) = mapping lookup (`DiscordAccountLinkService`, NOT payload-carried id) → `ReengagementApiClient.getPayload` → `DiscordOutboundService.sendProactivePayload` → `markSent`. **Never** wire `WebActivityService` (#595 gate) or the #596 consent filter into this path — dormant learner là recipient hợp lệ; consent filter thuộc #854 batch scan. Ambiguous send → markSent SUCCESS (anti-duplicate). Metric `discord_reengagement_send_total{outcome}`.
- **Re-engagement batch (#854):** cron `discord-reengagement-cron` — gate `REENGAGEMENT_ENABLED` (default off), advisory lock `DISCORD_REENGAGEMENT` 884_200_950 (contention → log+skip), scan 1 page `getCandidates` (không cursor; ceiling `REENGAGEMENT_MAX_PER_BATCH`), **consent filter bot-side** (`NotificationPreferenceService.findReportOptedInUserIds` — bảng preference nằm trong bot DB nên backend không lọc được), gap `REENGAGEMENT_SEND_GAP_MS` giữa các send, `REENGAGEMENT_DRY_RUN` = scan-only, 1 failure không dừng batch, dedupe authority = backend suppression (không local sent-state).
- **Discord report retry-dispatch (#521):** cron `discord-report-retry-dispatch` chạy 15 phút — parity với Messenger twin: `ReportCronLeaderService` gate + advisory lock `DISCORD_REPORT_RETRY_DISPATCH` 884_200_951 (contention → debug log+skip); **mọi orchestration outcome đều viết terminal-or-deferred**: `claimSkipped` → requeue không consuming retry (claim owner còn sống), `deferred` → markFailed + future `next_retry_at`, `windowClosed` → expire terminal — không còn job kẹt `processing` chờ stuck-reset churn; **bất biến lease < stuck-threshold**: `resetStuckProcessingJobs` dùng `2 × leaseMs` (slow send không bị reclaim cướp lease trước khi markSent).
- **Reschedule recovery (#464):** cron 5 phút chạy **chung 1 bảng global** trên cả 3 bots — advisory lock chia sẻ `RESCHEDULE_RECOVERY` 884_200_952 (contention → debug log+skip); pattern: `RescheduleRecoveryCronService` nhận optional `{pgLock, lockId}` từ app module factory (giống `ReportClaimStaleResetCronService`); không cấu hình → chạy unlocked (legacy).
...
- Query chat quota: `npm run chat-quota:status` (`--psid`, `--user-id`, `--date`, `--ops`); rebuild counter: `chat-quota:rebuild` (`--dry-run`). `chat_quota_events.aggregate_id` stores `sha256(userId)` for linked learners and `sha256(external id)` for anonymous turns (#637/#640) — hash the raw id before filtering; see `docs/data-minimization-audit.md`.
...
- Bounded LLM admission (#389): `LLM_EXECUTION_ENABLED=true`, `LLM_MAX_CONCURRENT` (default `3`), `LLM_MAX_QUEUE_DEPTH` (default `50`), wait budgets `LLM_ADMISSION_WAIT_MS` (chat, default `8000`) / `LLM_BACKGROUND_ADMISSION_WAIT_MS` (reports/reminders shed first, default `1500`) — **one contract for all 3 bots and all features**. Both execution paths share the `BoundedAdmissionQueue` core (`packages/llm-agent`) and throw typed `LlmOverloadError` (`queue_full|wait_timeout|global_saturated|redis_unavailable`) before invoking the provider; caller abort/deadline cancels admission including Redis-global acquisition; startup fails closed when `LLM_GLOBAL_CONCURRENCY_ENABLED=true` without Redis. Metrics: `<prefix>_llm_admission_rejected_total{reason}` + `_llm_admission_wait_seconds` + `_llm_admission_queue_depth` + `_llm_admission_drain_lag_seconds` + `_llm_concurrency_events_total{outcome}`. A single configured provider warns at startup because failover redundancy is unavailable; unknown or keyless providers fail closed. To shed load, keep execution enabled and lower the concurrency, queue-depth, and admission-wait budgets (background wait first); `LLM_EXECUTION_ENABLED=false` is an uncontrolled passthrough, not a hard stop. Multi-pod aggregate budget: `LLM_GLOBAL_CONCURRENCY_ENABLED=true` + Redis → one shared `llm:concurrency:global` slot across pods/bots. Redis slot acquisition (#453) uses full-jitter exponential backoff (base 50ms, cap 1s — no fixed cadence, no synchronized herd), bounded by the caller's admission budget, and fails fast with `redis_unavailable` after 3 consecutive Redis errors (~150ms) instead of burning the whole wait budget against a dead Redis.
...
- Retry jitter (#577): every retry delay many jobs can align on after one upstream failure goes through the shared equal-jitter `jitteredDelayMs(nominalMs, rng = Math.random)` (`packages/bot-common/src/utils/jitter.utils.ts`) → uniform in `[nominal/2, nominal)` (never exceeds nominal, so stays within the caller's cap; a caller needing a ceiling caps before the call). Covers both LLM retry layers (`AgentService.withRetry` caps at `MAX_RETRY_DELAY_MS`; `retryWithBackoff` caps at `LLM_OPENAI_RETRY_MAX_DELAY_MS` default 10s via `cappedExponentialBackoff`), WISPACE `withRetry`, and the durable `next_retry_at` writes in `webhook-inbound` `markFailed`, `StudyReminderDispatchService`, and `reportRetryAt` (`scheduler/application/utils`). `rng` seams (`RetryBackoffOptions.rng`, `WithRetryOptions.rng`, `InboundRetryConfig.rng`, `StudyReminderDispatchServiceOptions.rng`, `reportRetryAt(minutes, rng, now)`) make tests deterministic. Out of scope (inventory in §7.1.1): Redis slot acquire (#453, already full-jitter), dead-letter replay (coarse cron + bounded sequential drip), single-row token/link refreshes. Always-on, no flag. Details: `docs/project-overview.md` §7.1.1.
- LLM safety: free-form chat blocks prompt injection before calling LLM, sanitizes history/tool results; external data for reminders/reports must go through `prompt-injection.utils` / validate JSON output (`llm-json-output.utils`) before formatting/sending. **Safety telemetry never stores raw text** — `LlmSafetyCore.recordGroundingWarning` / `recordInjectionEvent` persist sanitized excerpt (control chars stripped, credential patterns → `[REDACTED]`) + SHA-256 hash + length only. **Reminder time** always renders the server-derived `scheduledTimeLabel` (model value ignored, mismatch logged). **Student-report facts** (streak, task statuses, dates, counts, bands) are generated deterministically from source data — the LLM only writes prose.
- Stored injection & history replay (#629): a learner-authored WISPACE free-text field (goal text, calendar title/notes, the progress `report`) that a tool reads back is neutralized by `sanitizeToolResultContent`; **every replayed history entry is re-sanitized with the same `sanitizeUntrustedTextForLlm` pipeline as fresh input** in the internal `ContextManager` used by `LlmAgentService` — a stored turn is never trusted because it "passed once". A neutralized injection from any of the three sources (`user_input` / `tool_result` / `history`) is metered via `LlmSafetyEventPort.recordInjectionEvent` → `llm_safety_events` `event_type='INJECTION_BLOCKED'` (redacted excerpt + `source` in the payload) and the `<prefix>_llm_injection_blocked_total{source,platform}` counter. `isInjectionSanitizeReason` gates it — benign `secret_redacted` / length trims do not fire an event. Residual (static-regex limitation, #336): a no-trigger-word instruction inside a free-text field is not redaction-caught; the `{ok,data}` envelope + the "tool data ≠ instructions" prompt rule + the no-fabrication / grounding guard are the mitigation.
...
- Send-API breaker accounting (#517): the fleet-wide Meta Send API breaker (`MessengerOutboundService`) and the wispace-client breakers (`createCircuitBreaker`) only trip on transport/5xx/timeout(408)/rate-limit(429) failures — deterministic per-user 4xx (expired 24h window, token errors, invalid recipients) never lock out healthy users. Classifier `shouldTripMessengerBreaker` / `classifyMessengerSendApiError` (messenger-outbound.service.ts); wispace-client excludes non-5xx `WispaceApiError` + aborts. Metrics: `<prefix>_send_api_circuit_breaker_state`, `<prefix>_send_api_circuit_failures_total{error_class}`, `<prefix>_send_api_circuit_events_total{action,cause}`.
...
- Production env sync: use the manual `sync-env.yml` workflow to refresh the per-bot Vault AppRole bootstrap; production bundles contain no full secret env and containers do not receive the host Docker socket.
...
- Health endpoints (shared `HealthController` in `packages/bot-common`, same semantics on all 3 bots): `GET /health` = **public liveness** (generic `{status:"ok"}`, never leaks dependency details); `GET /health/ready` = **public readiness** (200 only when DB, configured Redis, platform connectivity, and existing readiness checks are healthy; 503 body is status-only); `GET /health/detail` = **internal** (`X-Internal-Api-Key`) full DB/Redis/platform detail. Platform readiness is cached in each container and never performs a vendor call inside the probe: Discord uses gateway lifecycle state plus a 60s reconnect grace, Messenger uses startup/5-minute read-only Graph validation plus outbound token signals, and Zalo uses the OA token refresh lifecycle. Missing platform state fails closed. Deploy gates and `vps-self-pull-deploy.sh` use `/health/ready`; Nginx rate-limits public readiness probes.
- Redis R0: `REDIS_ENABLED=true` + `REDIS_*` → startup logs PING; shared commands use bounded `REDIS_COMMAND_TIMEOUT_MS` (default 2000ms) and connections use bounded `REDIS_CONNECT_TIMEOUT_MS` (default 5000ms), each validated from 1–60000ms; deadline failures surface as typed `RedisCommandTimeoutError` / `RedisConnectTimeoutError` and low-cardinality deadline metrics; disconnected commands reject instead of entering an offline queue and unfulfilled commands are not replayed after reconnect. Non-local Redis requires TLS, or explicit `REDIS_PRIVATE_NETWORK=true` on a private Docker/VPC network; readiness via `GET /health/ready` (503 when enabled but unreachable).
...
- **WISPACE read cache coherence** (#636/#568/#705): all cached WISPACE reads (goals, calendar, score averages) go through the single `WispaceDataCache` in `@wispace/wispace-client` — TTLs live ONLY in `WISPACE_CACHE_POLICY` (goals 60s, calendar 15s, scores 5min; a new read must add a kind there, it cannot opt out). Cache key = `kind + externalUserId + canonical args` (signal excluded). Write-invalidation: `precreate_next_exercise` success → `cacheInvalidation.invalidateGoals` (chat-agent `WispaceCacheInvalidationPort`); the shared `@wispace/reschedule-confirm` mutation boundary invokes an optional `CalendarCacheInvalidationPort.invalidateCalendar` only after the reschedule write and claim cleanup (staging does NOT invalidate); Discord/Zalo pass their existing adapters, Messenger has no calendar cache. Stampede protection (#568): concurrent misses share one in-flight fetch per key (failures are not cached — the next caller retries); with Redis enabled, D/Z wire a `RedisWispaceCacheStore` shared store (D/Z wispace modules) so pods coordinate one upstream fetch via a token-scoped lock (5s lease) with a mandatory second check after the lock grant, a bounded 2s waiter budget (then fail-open local fetch), ±10% shared-TTL jitter, date-safe envelope codec, and every shared op failing soft — Redis off/down is just the per-pod cache. `MemoizedWispaceGoalsService` is a thin facade over the same cache (report paths D/Z); scores stay passthrough there (report freshness unchanged).
...
- Auto-recovery crons: `chat-quota-stuck-recovery` (5 min, advisory-locked) refunds quota slots stuck `reserved` past `CHAT_IDEMPOTENCY_STUCK_RESERVED_MS` (linked rows decrement by `(platform, user_id, external_user_id, usage_date)`; anonymous rows by `(platform, external_user_id, usage_date)`); `report-claims-stale-reset` (30 min, advisory-locked) releases expired per-platform and learner-level scheduled-report leases on Messenger/Discord/Zalo. Claims receive a UUID `lease_token` + `lease_expires_at`; mark-sent/release require the current token, so stale workers no-op. Existing claimed rows with null lease fields use `updated_at` as the recovery cutoff. Released claims are **reclaimable** on the next claim (ON CONFLICT DO UPDATE `WHERE status='released'`), `sent` claims stay non-reclaimable.
...
- Study reminder sync requires an authoritative `getSessions` provider at every entry point (worker, ops controllers, relink, calendar-command) — missing provider fails closed with an error, never cancels jobs from an assumed-empty calendar. Full sync is **keyset-paged** (100 mappings/page, cursor by id) and processed with bounded concurrency (5) — memory and duration no longer grow with one serial upstream fetch per user (#130). Dispatch is **platform-scoped** (#180): `findDueJobs`/`claimJob`/`resetStuckProcessingJobs` take the worker's `platform` — a bot can never claim or reset another bot's jobs (the dispatch service binds its platform in the providers factory). Advisory lock ids for the worker's sync/cleanup/rollover crons are **per platform** (#777): Messenger `884_200_901/902/903` (its own app registry), Discord `884_200_944/945/946`, Zalo `884_200_947/948/949` (in `ADVISORY_LOCKS`, bot-common); `createStudyReminderProviders` fails closed without `workerLockIds`; lock skips are observable via `study_reminder_lock_skips_total{platform,scope}` + periodic-sync warn (the deliberately shared `DATA_QUALITY_CHECK` id, #688, is unrelated). Ownership generation fencing (#999): every job snapshots `mapping_generation`; relink/revoke/unlink cancels stale jobs, and dispatch verifies `(userId, generation)` under the shared per-external-id lock before the delivery-key write/provider call; legacy null generations fail closed.
- Reschedule is **create-before-delete**: the replacement slot is created first (idempotent — an existing target slot is reused on retry), then the source is deleted with bounded retries; a failure never leaves the user without a session. Confirmation staging uses a guarded `INSERT ... ON CONFLICT ... WHERE status <> 'processing'` and returns a conflict instead of overwriting an in-flight row; user cancellation is external-id-scoped and claimed cleanup requires the exact lease, so stale recovery cannot delete a replacement.
- Shared Discord/Zalo reschedule staging (#1062) forwards the optional caller `AbortSignal` through calendar lookup and pending save. Before confirmation delivery, an internal abort sends no prompt and cleans only the matching approval-token/nonce; a token-less stage fails closed. Cleanup is bounded and sanitized; after outbound delivery starts, existing semantics remain authoritative.
...
- **Prod hardening** (see `deploy/`): nightly `pg_dump` backup cron on VPS (`deploy/postgres-backup.sh`, 02:00, giữ 14 ngày) **encrypted at rest** with GPG AES-256 (`BACKUP_ENCRYPTION_PASSPHRASE`); hourly `deploy/backup-monitor.sh` checks backup age and fires Alertmanager alert if >25h stale; deploy tự chạy migrations (advisory-locked, `MIGRATION_CMD`) + health check (`health_path`) + tự rollback về image cũ nếu không healthy; Prometheus scrape cả 3 bot + Alertmanager (#683 severity routing: critical → Discord (embed @here marker, không hard-ping vì AM 0.27 Discord receiver chỉ post embeds) + Pushover emergency (retry 5m/expire 1h — interrupt chính) + Telegram; warning → Discord không-ping; 4 credentials mới fail-closed trong `monitoring/.env`: `DISCORD_ALERT_WEBHOOK_*_URL`, `PUSHOVER_USER_KEY`, `PUSHOVER_API_TOKEN`; `AlertDeliveryFailed` alert trên `alertmanager_notifications_failed_total` + `deploy/monitoring/synthetic-alert.sh` cron probe daily-warning/weekly-critical; #515 external deadman: alert `Watchdog` luôn firing → AM intercept route (`alertname="Watchdog"`, đặt đầu, không fan-out, `send_resolved: false`) ping healthchecks.io mỗi 2m (`HEALTHCHECKS_PING_URL` fail-closed, period 10m/grace 5m → page ≤15m qua Pushover+email của chính healthchecks.io) + cron-job.org GET `/health/ready` mỗi 5m → check #2 (period 30m/grace 15m); runbook blackout scenario trong `docs/monitoring-alerts.md`). Prometheus and bare blue-green bots share the external `monitoring` network; stable `<app>-metrics` aliases point to fixed internal ports while Nginx owns host-port switching.
...
npm run database:privacy-smoke            # real cross-platform privacy unlink/delete/export (legacy registry smoke)
...
- Spec-style JSON fixtures in `packages/llm-agent/fixtures/*.json` — each declares input (`coreHash` (sha256 of the LF-normalized `CHAT_SYSTEM_PROMPT_CORE` **value** — the exact text production composes), `promptFiles` (path + sha256 per overlay prompt file, LF-normalized), optional `history`/`systemPromptSuffix`) + a per-round LLM script (tool calls with args + tool results — optional `content` = the model's plan line on multi-intent tool rounds — or final text) + `expected` (tool order, `toolSummary`/`exhausted` flags, `replyTextContains`/`replyTextNotContains` fabrication guard, `groundingWarnings`, `injectionEvents` (#629 — neutralized tool-result/history injections), `planRemainder`, `requestContracts`). The multi-tool fixture (`multi-tool-calendar-then-exercise.json`) is the plan-step baseline for #207 item 2: its plan `content` must survive into the next round's messages (asserted by the harness).
...
- **Prompt changes fail the eval**: fixtures pin the real chat prompts — `coreHash` (sha256 of the `CHAT_SYSTEM_PROMPT_CORE` value, LF-normalized so Windows CRLF checkouts match CI) + overlay files by path + sha256 — any prompt edit breaks the hash until the fixture behavior is re-validated and the hash is updated deliberately (no auto-approve). Runtime and eval compose through the single `composeChatSystemPrompt` (#646) — part order, separator, and suffix handling live in one place. After a deliberate prompt edit, build `@wispace/llm-agent`, run `npm run eval:rehash` (or `npm run eval:rehash:check` to verify without writing), then run `npm run eval:chat`/the relevant tests to re-validate fixture behavior. The guardrail CI workflow runs the check mode and never writes fixtures.
- **Guardrail battery (#635)** — `npm run eval:guardrail --workspace=@wispace/llm-agent` runs ALL eval fixtures as one tiered CI check (required workflow `.github/workflows/guardrail-battery.yml`, path-filtered to `packages/llm-agent/**`, `packages/chat-agent/src/agent/**`, `apps/*/src/shared/prompts/**`). Tier is a fixture field: `tier: "adversarial"` on the injection/disclosure probes (15 today); everything else defaults to `must-block`. Pass bar: must-block requires **100%**; adversarial tolerates a **10% bypass rate** (`ADVERSARIAL_BYPASS_RATE_LIMIT` in `guardrail-battery.ts`). Output reports each tier's rate and names every regressed fixture. **Red-then-green process for a new bypass:** (1) capture the bypass as a new fixture with `tier: "adversarial"` and its scripted reply — CI goes red; (2) land the fix — the fixture turns green and the reported rate drops; (3) once the fix is proven, promote the fixture to must-block (remove the tier field) so it can never regress. Do not delete a red fixture instead of fixing the guard.
- Non-disclosure guard (#625): `disclosure-*.json` (empty script → `detectDisclosureProbe` deflects to the standard line before the LLM), `disclosure-output-*.json` (scripted leaky model text → `checkFinalOutputSafety` redacts), `control-are-you-ai.json` / `control-writing-scoring.json` (legit asks still reach the LLM). Taxonomy H (progressive multi-turn) and the debounce-split part of G are **out of scope for the offline harness** (scripted model, no multi-turn reasoning) — covered by the prompt core only.
...
| LLM input classifier tier (#649)                | `.claude/rules/prompts.md` (Non-disclosure guard section), `docs/project-overview.md` §7 + §8, `apps/messenger-bot/.env.example`, `packages/llm-agent/src/classifier/classifier-eval.fixtures.ts` + `.github/workflows/classifier-eval-nightly.yml` (quality lane, #505) |
| Deploy / CI / VPS path                          | `.github/workflows/deploy-bots.yml` + `deploy-bot-reusable.yml` + `deploy/Dockerfile.bot` (shared, `ARG APP_NAME`), `apps/messenger-bot/docs/scale-phase-b-runbook.md`, `deploy/nginx/`                                                                                  |
...
Tool execution: `PlatformAgentService` consumes `PlatformToolExecutorPort` (`packages/chat-agent`) — the shared `PlatformAgentToolsService` implements the Discord/Zalo tool sets; Messenger owns its executor (`MessengerAgentToolsService` implements the same port, wired via `useExisting` — no `toolOverrides` conditional dispatcher). WISPACE data access goes through narrow capability ports (`packages/chat-agent/src/agent/wispace-capability.ports.ts`, #425) — chat-agent must not import `wispace-client` (CI check `.github/scripts/check-chat-agent-wispace-imports.sh`); Discord/Zalo wire adapter wrappers and bake their identity headers at the composition root. Study-reminder worker wiring is a named typed seam (`createStudyReminderWorker(deps, config)` — no positional `deps[6]`/`unknown`; `createStudyReminderProviders` options take typed `ClassOf<T>` constructors, and the worker's session source is injected via the required `GET_SESSIONS` token — Discord/Zalo bridge with `createCalendarGetSessions`, Messenger with `createSessionSourceGetSessions`; `study-reminder-shared` must not reference the concrete WISPACE client classes — CI check `.github/scripts/check-study-reminder-shared-wispace-imports.sh`, #424). App-layer ports: `REMINDER_STUDENT_DATA_PORT` (messenger study-reminder), `DISCORD_ACCOUNT_LINK_REPOSITORY` + `DISCORD_OAUTH_STATE_REPOSITORY` + `DISCORD_OAUTH_EXCHANGE` + `DISCORD_GUILD_MEMBERSHIP` (discord account-link — OAuth state persistence, HTTP exchange, and guild check are behind ports; adapters wired in `AccountLinkModule`, #428), `DISCORD_REPORT_ACCOUNT_READER` (discord report cron + delivery/retry — single-link lookups go through `findLinkStateByExternalUserId`), `WEBHOOK_INBOUND_EVENTS_PORT` (messenger durable inbox) — concrete TypeORM/WISPACE implementations bound in module wiring. Messenger calendar commands use the shared `PlatformStudyCalendarCommandService` through an outer facade.
...
| Outbound learner-message backstop (#622) | `OutboundRateLimiter` in `@wispace/bot-common`; delivery adapters in Messenger/Discord/Zalo; [outbound-rate-limit.md](docs/outbound-rate-limit.md)                       |
...
cleanup makes three bounded attempts per store and returns `status: complete`
or `status: incomplete` with an opaque `cleanupId` and `outstandingStores`;
incomplete responses are HTTP 202, while generation conflicts are HTTP 409.
Each bot reconciles only its own platform jobs every five minutes (maximum 100,
60-second lease), fences relinks, and retains completed/stale jobs for seven
days. See [ADR-0014](docs/adr/0014-privacy-erasure-completion.md) and the
[verification drill](docs/privacy-erasure-verification.md).
...
- `discord_welcome_records`, `zalo_welcome_records` — dedupe state, no PII
- `scheduled_report_claims` + `learner_scheduled_report_claims` — report audit/claim state, cleaned by the report-claims retention cron
...
**Redis cleanup**: each bot's ops controller (and Messenger's in-chat privacy-intent path) wires per-call `PrivacyStateCleanup` callbacks on every `unlink()`/`delete()` call, using the app's own already-correct, already-existing services — `clearHistory` via `PlatformChatHistoryService.clear`, `clearQueuedWork` via the chat-queue service's `clearChatBuffer` (per-user `srem`/`zrem` on the shared active/flush/stuck sets, never a bulk `DEL` of them), `clearClarification` via the agent's clarification clearer, and `clearUserCache` (Messenger only — Discord/Zalo have no display-name cache) via `RedisUserDisplayNameCache.delStrict(userId)` on the durable privacy path (`del()` remains best-effort for non-privacy callers). Callbacks clear **only this app's platform keys** — cross-platform Redis erasure requires the backend to call each bot's `/privacy/delete` endpoint (idempotent); calling one platform's callbacks with another platform's id is a safe no-op (#537), never destructive, since key shapes never collide across platforms. Durable jobs make failures retryable after the request; memory-mode history still clears only the local pod (other pods rely on TTL/restart). Dequeue race (in-flight turn replies after erasure) is accepted — window is sub-second. The real PostgreSQL/Redis fleet drill is automated by `npm run database:privacy-erasure-drill` and documented in `docs/privacy-erasure-verification.md`.
...
| —        | [docs/slo-catalog.md](docs/slo-catalog.md)                                                             | SLO catalog + error-budget policy for 3 learner journeys    |
...
| PostgreSQL HA/failover (#408)                                                       | ✓ Writer endpoint guard, bounded query timeout, migration fencing, backup writer preflight, and runbook; managed provider standby/PITR provisioning + measured staging drill remain operational work                                                                                                                                                                                                                                                     |
| Upsert `sent` job when rescheduling same `session_key`                              | ✓ `StudyReminderJobRepository.upsertPendingJobs` reopens → `pending`                                                                                                                                                                                                                                                                                                                                                                                     |
...
| Messenger verify-intent durability (#821)                                           | ✓ Ref fingerprint; one active generation per PSID; persistence-reserved lease fenced before mapping/side effects; five-minute stale reconciliation and committed cleanup                                                                                                                                                                                                                                                                                 |
| Multi-pod 08:00 report cron (R4)                                                    | ✓ Claim table + advisory lock + `CRON_LEADER_ENABLED`; lock id + lease name are **platform-scoped** (#510) — Messenger `884_200_801` / Discord `884_200_802` / Zalo `884_200_803` (in `ADVISORY_LOCKS`), lease `report:<platform>`, Zalo now wired too                                                                                                                                                                                                   |
| Two-way chat + rate limit V1                                                        | ✓ Reserve/refund/burst/whitelist/hint                                                                                                                                                                                                                                                                                                                                                                                                                    |
| Standalone chat intent routing (#941)                                               | ✓ Shared full-message greeting/self-intro matcher; content-bearing text reaches normal chat on Messenger/Discord/Zalo                                                                                                                                                                                                                                                                                                                                    |
| Clarification choice replay (#1035)                                                 | ✓ Consumed state suppresses only the same event identity; new or id-less choices reach the agent, failed choice delivery is retryable, and `skip_delivery` is a separate clarification outcome                                                                                                                                                                                                                                                           |
| Rate limit hardening H1–H7                                                          | ✓ H2–H7 code; H1 = enable `CHAT_RATE_LIMIT_ENABLED` on prod env                                                                                                                                                                                                                                                                                                                                                                                          |
| Postgres/Redis consistency (#609)                                                   | ✓ Postgres-final burst enforcement; bounded Redis audit/invalidation; queue index repair + malformed-state quarantine; low-cardinality drift metrics and alert — [ADR-0007](docs/adr/0007-postgres-redis-consistency.md)                                                                                                                                                                                                                                 |
| MVCC / concurrency direction (#576)                                                 | ✓ Decision only — `READ COMMITTED` baseline, no generic app-level MVCC layer; per-flow mechanism (single-statement CAS, lease token, `FOR UPDATE`, one `version` CAS on `zalo_oa_tokens`, advisory lock, Redis Lua) + read-your-writes rules inventoried; no race found, no code changed — [ADR-0008](docs/adr/0008-mvcc-and-optimistic-concurrency.md)                                                                                                  |
| Per-learner outbound message backstop (#622)                                        | ✓ Shared rolling Redis cap, atomic batches, terminal `rate_limited` handling, low-cardinality metrics, and runbook                                                                                                                                                                                                                                                                                                                                       |
| Multi-platform learner consistency (#637)                                           | ✓ Canonical WISPACE `userId`; linked daily quota is shared, reminders keep one canonical owner, scheduled reports use one atomic learner/date/type claim, and unlink is channel-local while delete is userId-scoped                                                                                                                                                                                                                                      |
| Privacy erasure recovery (#995/#1128–#1131)                                         | ✓ Durable `privacy_cleanup_jobs`, truthful 200/202/409 outcomes, own-platform five-minute reconcilers, generation fencing, metrics/Ops Health alerts, and seven-day terminal retention — [ADR-0014](docs/adr/0014-privacy-erasure-completion.md) + [verification drill](docs/privacy-erasure-verification.md)                                                                                                                                            |
| Batch canonical-platform lookup (#954)                                              | ✓ Messenger/Discord/Zalo report crons resolve canonical ownership with one set-based PostgreSQL query per keyset page; duplicate learner IDs are deduplicated, active/preferred-platform semantics are shared, and lookup errors fail closed without an O(n) fallback                                                                                                                                                                                    |
| LLM Provider Abstraction                                                            | ✓ Adapter pattern (`LlmProviderAdapter`), OpenAI adapter, OpenAI-compatible adapter, factory — PR #32                                                                                                                                                                                                                                                                                                                                                    |
| Shared LLM execution resilience (#513)                                              | ✓ Discord/Zalo env port and Messenger use bounded admission + breaker + configured failover; single-provider startup warning; Redis lifecycle, queue-depth, and drain-lag metrics                                                                                                                                                                                                                                                                        |
| Fail-closed LLM provider configuration (#1075)                                      | ✓ Shared exact-host endpoint/model policy validates every active provider and failover candidate at startup; Messenger aliases and classifier ambiguity fail closed (#1104–#1106)                                                                                                                                                                                                                                                                        |
| Canonical LLM safety matching (#501)                                                | ✓ Shared raw + canonical candidates cover input/history/tool scans and chat output prompt/vendor checks; credentials remain raw-only; must-block eval fixtures pin separator/format-character regressions                                                                                                                                                                                                                                     |
| Discord outbound action guard (#633)                                                | ✓ Shared send/edit boundary sets restrictive `allowedMentions`, neutralizes model-generated Discord mention tokens, and records masked-id telemetry; Zalo/Messenger audit found no equivalent action markup                                                                                                                                                                                                                                              |
| Generic LLM tool capability/approval policy (#416)                                  | ✓ Shared capability registry + fail-closed identity checks; explicit-intent/idempotency paths; reschedule approval bound to platform, mapping revision, intent/args, nonce, and expiry                                                                                                                                                                                                                                                                   |
| Non-identity calendar tool args (#627)                                              | ✓ List-first, caller-scoped calendar reads plus shared ownership checks before reschedule stage/write; scope denials are generic, masked, and metered (`scope_mismatch`/`scope_unverified`). WISPACE per-endpoint authorization confirmation remains external issue evidence                                                                                                                                                                             | Done (code) |
| Reschedule confirmation safety (#1037)                                              | ✓ Stop/cancel clears pending platform-scoped proposals; Messenger/Discord approvals remain token-bound buttons; Zalo requires exact tokenized confirmation; bare affirmatives are rejected; expiry is lazy/explicit; failed proposal delivery disarms the staged request                                                                                                                                                                                 | Done        |
...
| Canonical study-reminder owner (#718)                                               | ✓ Messenger/Discord/Zalo full-sync providers require `CanonicalPlatformService`; noncanonical sync cancels only `pending` / `failed`, leaves `processing`, and converges after ownership changes                                                                                                                                                                                                                                                         |
| Study-reminder ownership fence (#999)                                               | ✓ Jobs snapshot `mapping_generation`; relink/revoke/unlink cancellation and dispatch's per-external lock prevent an old mapping from writing a delivery key or sending after ownership changes; legacy null generations fail closed                                                                                                                                                                                                                      |
...
| Explicit opt-in for scheduled reports/reminders (#596/#942)                         | ✓ `user_notification_preferences.report_enabled` (opt-in, NULL = off) + `.reminder_enabled` (opt-out, NULL = on); D/Z report crons + reminder mapping readers filter on them; Messenger selection + sync/dispatch real-Postgres smoke (#942); deterministic chat commands (pre-LLM) on all 3 bots; post-link explainer once/platform (`optin_prompt_sent_at`); grandfather migration; `forceSend` bypass; opt-out footer 1 lần (`optout_notice_sent_at`) |
...
| TypeORM migration timestamp collisions (#340)                                       | ✓ Runtime class/effective-name timestamps are unique; legacy rows for the four historical names are recognized through the guarded migration compatibility map without rewriting the `migrations` table; CI checks filename, class, and effective-name collisions                                                                                                                                                                                        |
