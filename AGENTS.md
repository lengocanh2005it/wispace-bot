...
- Outbound retry policy (#156): Discord/Zalo sends retry only rate limits/5xx and explicit network failures; known 4xx/auth/validation errors fail fast. Timeout/ambiguous delivery outcomes are not retried and increment the `dm_send_ambiguous` metric to make possible provider acceptance observable without creating duplicate messages.
