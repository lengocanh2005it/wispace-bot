...
- **Postgres offsite recovery (#866):** the Messenger deploy workflow reads the shared Vault path and installs a separate mode-600 host-only `backup-bootstrap.env` at `/home/ngoc_anh/backups/ai_chat_bot_db/backup.env`; it is never passed to bot containers. `postgres-backup.sh` and `postgres-offsite-sync.sh` use that file; offsite sync promotes checksum-verified encrypted dump/globals/pre-migration/evidence plus a manifest through rclone and never deletes remote objects. `backup-monitor.sh` checks the remote manifest/object sidecars hourly and downloads the remote pair for guarded restore verification weekly. Bucket versioning/Object Lock and lifecycle retention are operator prerequisites.
