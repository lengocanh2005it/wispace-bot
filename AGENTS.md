...
- **PgBouncer (production, optional):** `deploy/docker-compose.pgbouncer.yml` uses the pinned `edoburu/pgbouncer` image and generates auth files from `PGBOUNCER_DB_USER`/`PGBOUNCER_DB_PASSWORD`; it joins both `app_n8n_db_network` and `monitoring`. Bots use `DB_HOST=pgbouncer`, `DB_PORT=5432`, and `DB_ALLOW_INSECURE_HOSTS=pgbouncer`. Keep `pool_mode=session` because advisory locks require one backend connection per client session; do not add unsupported `auth_password` to `pgbouncer.ini`.
