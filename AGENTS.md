...
- **Discord linking commits at OAuth callback, independent of guild membership**: verify token → persist verify-intent (`discord_link_verify_records`, durable outbox #137) → `upsertLink` (retry — token đã bị WISPACE consume) → consume intent (fire-and-forget) → đã trong guild? gửi welcome DM + `markWelcomed` : redirect thẳng `DISCORD_INVITE_URL` (FE không có trang callback, không đọc param nào). `guildMemberAdd` chỉ gửi welcome cho user đã link (qua `findUserIdByDiscordId`) **nếu chưa được welcome trong window** (`DISCORD_REWELCOME_WINDOW_MS`, default 24h — `last_welcomed_at` trên mapping dedupe re-join + join-during-callback race); user chưa link vẫn nhận DM chào organic. Crash giữa verify và upsert được reconcile bởi cron `discord-link-reconcile` (5 phút, advisory lock `DISCORD_LINK_RECONCILE` 884_200_934; `DISCORD_LINK_RECONCILE_AGE_MS`/`DISCORD_LINK_RECONCILE_MAX_AGE_MS`). Relink (cùng Discord ID sang WISPACE user khác) → DM thông báo cho account + log warning (user cũ bị thay mapping — by design). DM bị chặn (privacy) → counter `discord_dm_delivery_failures_total{reason}` (Prometheus). Redirect luôn kèm `Referrer-Policy: no-referrer`; landing là `DISCORD_LINK_LANDING_URL`. Portal UI phân biệt "linked + chưa join" qua `GET /v1/discord/link-status?userId=` (guard `X-Internal-Api-Key`, trả `{linked, inGuild}` — xem `DiscordLinkStatusController`).
