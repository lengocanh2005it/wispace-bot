...
- **Discord linking commits at OAuth callback, independent of guild membership**: verify token → `upsertLink` (retry — token đã bị WISPACE consume) → đã trong guild? gửi welcome DM : redirect thẳng `DISCORD_INVITE_URL` (FE không có trang callback, không đọc param nào). `guildMemberAdd` chỉ gửi welcome cho user đã link (qua `findUserIdByDiscordId`), không có pending/cookie/join-status nữa. Redirect luôn kèm `Referrer-Policy: no-referrer`; landing là `DISCORD_LINK_LANDING_URL`. Portal UI phân biệt "linked + chưa join" qua `GET /v1/discord/link-status?userId=` (guard `X-Internal-Api-Key`, trả `{linked, inGuild}` — xem `DiscordLinkStatusController`).
