...
| Explicit opt-in for scheduled reports/reminders (#596)                              | ✓ `user_notification_preferences.report_enabled` (opt-in, NULL = off) + `.reminder_enabled` (opt-out, NULL = on); D/Z report crons + reminder mapping readers filter on them; deterministic chat commands (pre-LLM) on all 3 bots; post-link explainer once/platform (`optin_prompt_sent_at`); grandfather migration; `forceSend` bypass; opt-out footer 1 lần (`optout_notice_sent_at`) |
