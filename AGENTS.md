...
- Query chat quota: `npm run chat-quota:status` (`--psid`, `--user-id`, `--date`, `--ops`); rebuild counter: `chat-quota:rebuild` (`--dry-run`). `chat_quota_events.aggregate_id` stores `sha256(psid)` (#640) — hash the raw id before filtering; see `docs/data-minimization-audit.md`.
