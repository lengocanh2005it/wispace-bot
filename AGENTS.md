...
- Retry jitter (#577): every retry delay many jobs can align on after one upstream failure goes through the shared equal-jitter `jitteredDelayMs(nominalMs, rng = Math.random)` (`packages/bot-common/src/utils/jitter.utils.ts`) → uniform in `[nominal/2, nominal)`. Covers `retryWithBackoff` (`llm-agent`, + optional `maxDelayMs` cap), WISPACE `withRetry`, and the durable `next_retry_at` writes in `webhook-inbound` `markFailed`, `StudyReminderDispatchService`, and `reportRetryAt` (report-send outbox). `rng` seams (`InboundRetryConfig.rng`, `StudyReminderDispatchServiceOptions.rng`, `reportRetryAt(minutes, rng, now)`) make tests deterministic. Redis slot acquire (#453) already full-jitter, unchanged. Always-on, no flag. Details: `docs/project-overview.md` §7.1.1.
