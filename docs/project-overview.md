...
- **Redis (provisioned in production; `REDIS_ENABLED` flag retained):** `REDIS_ENABLED`, `REDIS_HOST`, `REDIS_PORT`, `REDIS_PASSWORD`, `REDIS_TLS`, `REDIS_CA`, `REDIS_PRIVATE_NETWORK` — R0–R4 stores + R5 user display cache; readiness via `GET /health/ready` when enabled
