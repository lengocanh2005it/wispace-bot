...
- **Discord/Zalo queued failures:** a failure before the main reply is delivered sends one direct generic Vietnamese fallback through the outbound service. The fallback never re-enters the chat queue; original pipeline failures and fallback delivery failures are logged separately. Outbound sends retry only rate limits/5xx and explicit network failures; known 4xx/auth/validation errors fail fast, while timeout/ambiguous outcomes are recorded as `dm_send_ambiguous` and are not retried to avoid duplicates (#156).
