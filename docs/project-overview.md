...
- **Privacy/relink:** unlink invalidates only the requested channel and its
  ...
  identity never transfers quota, report, or reminder state. Platform-owned
  Redis cleanup is durable (`privacy_cleanup_jobs`): requests report `complete`
  (HTTP 200) or `incomplete` (HTTP 202 with an opaque cleanup id and outstanding
  stores), while generation conflicts remain HTTP 409; each bot reconciles only
  its own jobs.
...
- **Clarification follow-ups (#401, #1035):** ambiguous or contradictory chat is answered with a bounded menu and never reaches the LLM/tools until one choice is selected. State is keyed by platform + external user id and bounded by `CHAT_CLARIFICATION_TTL_MS`, `CHAT_CLARIFICATION_MAX_ATTEMPTS`, and `CHAT_CLARIFICATION_MAX_MENU_RESETS`; recent event ids form a short tombstone so delayed/replayed choices cannot execute tools. A consumed state suppresses only a replay of a known event identity: a new `correlationId` (or no id) with the same text is a fresh turn, while the same id is returned with `skipDelivery` and never reaches the LLM. Non-ambiguous delivery failure reopens the consumed event for retry; `<prefix>_clarification_outcomes_total{outcome=skip_delivery}` counts suppressed sends. Redis parsing uses hard safety caps and the absolute TTL ceiling, so lowering runtime limits does not discard still-live state. Definitive clarification-delivery failures are persisted in the outbound dead-letter table with a stable `deliveryKey` for the platform retry cron; ambiguous transport failures are not auto-resend. **Stop/resume words (#959):** a recognized stop request (`dừng`, `thôi`, `stop`, `hủy`, `khoan`, `không cần nữa`, `thôi khỏi`) is answered with the shared stop acknowledgement (`buildStopAcknowledgedMessage`) instead of the menu, and clears any pending menu state; `tiếp` / `tiếp tục` are not ambiguous either. Short meaningful input (`7.0`, `band`, `6.5?`) escapes the four-character ambiguity gate via an explicit vocabulary, so genuinely vague input (random punctuation, single emoji) still gets the menu. A delivered clarification/stop reply is refunded in the pipeline (`reply.clarification` → `rateLimiter.refund`) and never marks the idempotency row delivered — a turn that answers nothing costs no quota (#661).
- **In-chat privacy confirm (#660, Messenger only):** `MessengerChatProcessorService` intercepts only short, explicit unlink / delete / export requests matched by `detectPrivacyIntent` **before** the quota block, so arbitrary essay text and privacy questions reach the normal chat pipeline. While a pending action exists (`PrivacyStateService`, in-memory, TTL `PRIVACY_CONFIRM_TTL_MS`, default 30 min), an action-specific confirmation phrase is required (for example, `Đồng ý xóa dữ liệu`); bare acknowledgements such as `ok` never execute the action. Vietnamese and English confirmation/cancellation forms accept no-diacritic text, punctuation, and polite suffixes. An unrelated reply clears the pending action and falls through to the normal pipeline; a new explicit privacy request replaces the pending action. The inbound consent/cancel is written to `message_logs` (`PRIVACY_CONFIRM_IN` / `PRIVACY_CANCEL_IN`) before the irreversible step. After TTL expiry a reply also falls through to the normal pipeline. Durable / cross-pod persistence of the pending state is #542.
- **Intent routing (#941):** The shared `@wispace/llm-agent` matcher recognizes only whole-message greetings and self-introductions (including canonical punctuation/diacritic variants). Content-bearing greetings, subject-specific introductions, and distress messages continue through the normal chat path on all three bots.
...
- Reschedule approvals are one-time, expiring, platform-scoped records bound to the linked WISPACE user, mapping revision, intent, canonical arguments, and nonce. Old rows are invalidated by migration `1786932000000`; current platform identity is required at confirmation. Staging refuses to replace a `processing` confirmation; user cancellation is scoped to its external identity, and post-confirm cleanup requires the exact processing lease. Stop/cancel clears a still-pending proposal, while a raced processing claim is reported honestly; Messenger/Discord buttons and Zalo tokenized text are the only approval paths, and bare acknowledgements never authorize a calendar write (#1037). Expiry is reported lazily on the next related interaction, and failed or ambiguous proposal delivery removes the staged row. Discord/Zalo staging also forwards the optional caller abort signal; before confirmation delivery, late cleanup is nonce-conditional and a token-less shared stage fails closed (#1062).
...
| `discord_link_verify_records`     | Durable generation/observation-fenced verify-intent outbox — reconciled by the `discord-link-reconcile` cron (#137, #467)                    |
...
| `privacy_cleanup_jobs`            | Durable own-platform cleanup outbox for Redis/state erasure; deterministic per operation, identity, generation, and store (#995)             |

Migrations include `1717747200008-CreateMessengerUsersCacheTable`,
`1786940300000-CreateZaloWelcomeRecordsTable`, and the additive
`1789093700000-CreatePrivacyCleanupJobs` migration (no backfill; migration
barrier runs it before application cutover).
...
| POST   | `/v1/messenger/privacy/unlink`                  | `{ "externalUserId": string, "expectedMapping"?: { exists, userId?, mappingGeneration? } }` | Durable unlink: 200 `complete`, 202 `incomplete` (`cleanupId`, `outstandingStores`), or 409 generation conflict; `unlinked` is the DB-mutation boolean |
| POST   | `/v1/messenger/privacy/delete`                  | `{ "externalUserId": string, "expectedMapping"?: { exists, userId?, mappingGeneration? } }` | Durable delete with the same 200/202/409 contract; `deleted` remains the DB-mutation boolean |
| POST   | `/v1/discord/privacy/unlink`                    | `{ "externalUserId": string, "expectedMapping"?: { exists, userId?, mappingGeneration? } }` | Discord own-platform durable unlink with the common 200/202/409 outcome |
| POST   | `/v1/discord/privacy/delete`                    | `{ "externalUserId": string, "expectedMapping"?: { exists, userId?, mappingGeneration? } }` | Discord own-platform durable delete with the common 200/202/409 outcome |
| POST   | `/v1/zalo/privacy/unlink`                       | `{ "externalUserId": string, "expectedMapping"?: { exists, userId?, mappingGeneration? } }` | Zalo own-platform durable unlink with the common 200/202/409 outcome |
| POST   | `/v1/zalo/privacy/delete`                       | `{ "externalUserId": string, "expectedMapping"?: { exists, userId?, mappingGeneration? } }` | Zalo own-platform durable delete with the common 200/202/409 outcome |
...
| GET    | `/v1/messenger/ops/llm-usage/summary`           | Query: `psid` **or** `userId`; `from`/`to` (YYYY-MM-DD, default today; range capped at `LLM_USAGE_RETENTION_DAYS`, 400 beyond) | Total tokens + estimated USD per feature for one student                                                                                                                                          |
...
| GET    | `/health/ready`                                 | —                                                                      | **Public readiness** — 200 only when DB, configured Redis, platform connectivity, and existing readiness checks are healthy; 503 `{ "status": "error" }` status-only (deploy gate uses this path) |
| GET    | `/health/detail`                                | —                                                                      | **Internal** (`X-Internal-Api-Key`) — full DB/Redis/platform connection detail for ops                                                                                                            |
...
Privacy cleanup adds low-cardinality attempt counters and pending/oldest-age
gauges. Ops Health warns on incomplete requests and raises a critical alert
when pending work is older than 15 minutes or repeatedly retrying.
...
- Privacy: `web_activity` rows are deleted by `PrivacyDataService.delete()` (userId-scoped). A row can be orphaned if the erased mapping had no `userId`, or re-created if WISPACE keeps pinging a since-erased learner — inert (the gate only reads userIds from active mappings), no cleanup cron. Platform-owned Redis/state cleanup is handled by the durable `privacy_cleanup_jobs` reconciler described in [ADR-0014](adr/0014-privacy-erasure-completion.md).
...
| `zalo-link-reconcile`               | `*/5 * * * *`                                 | `ZaloLinkReconcileCronService` — re-commit missing Zalo mappings from `zalo_link_verify_records` (advisory lock `884200937`; `ZALO_LINK_RECONCILE_AGE_MS`/`ZALO_LINK_RECONCILE_MAX_AGE_MS`)                                                       |
| `privacy-cleanup-reconcile`         | `*/5 * * * *`                                 | Own-platform `PrivacyCleanupReconciler` — claims at most 100 jobs with a 60-second lease, fences relinks, and prunes completed/stale rows after seven days (Messenger/Discord/Zalo)                                                               |
...
App-owned system prompts are in `src/shared/prompts/*.system.txt`, loaded via `load-system-prompt.ts`. Nest copies those assets to `dist/shared/prompts/` on build (`nest-cli.json` → `assets`). The student-report prompt has one canonical source in `packages/student-report/src/prompts/student-report.system.txt`; each bot build copies the same bytes to `dist/shared/prompts/` for its existing runtime loader.

The model returns `StudentReportProse` containing only the report headline. The final `StudentCapacityReport` adds factual fields derived deterministically from source data; those fields are not accepted from model output.

| File                        | Used by                                                                 |
| --------------------------- | ----------------------------------------------------------------------- |
| `packages/student-report/src/prompts/student-report.system.txt` | Student Report package; copied into each bot's `dist/shared/prompts/` |
...
configured or startup fails closed. A single configured provider emits a
startup warning because failover redundancy is unavailable.

While `LLM_EXECUTION_ENABLED=true`, all `chat.completions.create` calls go through **bounded admission control** (#389) — Messenger's `LlmExecutionService` and the shared env-driven port (`createEnvLlmExecutionPort` in `packages/llm-agent`, used by Discord/Zalo chat + reports) both run on one `BoundedAdmissionQueue` core: `LLM_MAX_CONCURRENT` caps in-flight provider calls, `LLM_MAX_QUEUE_DEPTH` (default 50) hard-caps the wait queue, and `LLM_ADMISSION_WAIT_MS` (default 8000, chat) / `LLM_BACKGROUND_ADMISSION_WAIT_MS` (default 1500, reports/reminders — background sheds first) bound how long a call may wait before a typed `LlmOverloadError` (`queue_full` | `wait_timeout` | `global_saturated` | `redis_unavailable`) is thrown before the provider is ever invoked. Retries 429/5xx (`LLM_OPENAI_RETRY_*`) and per-request deadline (`LLM_REQUEST_TIMEOUT_MS`) still **abort the in-flight request** (composed with the caller's AbortSignal), not just the wrapper promise; caller cancellation also cancels admission waiting including Redis-global slot acquisition. When `LLM_EXECUTION_ENABLED=false`, the execution wrapper invokes its callback directly, bypassing the admission cap, request deadline, retry budget, and circuit breaker; it is not a load-shed or hard-stop switch. Normal bot wiring binds an unconfigured adapter for the deterministic fallback, but the flag also skips active-provider policy validation. Use the controls below for load shedding. Scaling ≥2 pods: set `LLM_GLOBAL_CONCURRENCY_ENABLED=true` (+ `REDIS_ENABLED`) for one Redis-distributed aggregate budget (key `llm:concurrency:global`) shared across all pods and bots — startup fails closed when the flag is set without Redis. Metrics: `<prefix>_llm_admission_rejected_total{reason}`, `<prefix>_llm_admission_wait_seconds`, `<prefix>_llm_admission_queue_depth`, `<prefix>_llm_admission_drain_lag_seconds`, `<prefix>_llm_concurrency_events_total{outcome}`.

For incident load shedding, keep execution enabled and lower `LLM_MAX_CONCURRENT`,
`LLM_MAX_QUEUE_DEPTH`, `LLM_ADMISSION_WAIT_MS`, and
`LLM_BACKGROUND_ADMISSION_WAIT_MS` (lower the background budget first so
reports/reminders shed before chat). With Redis-global concurrency enabled,
also lower `LLM_GLOBAL_MAX_CONCURRENT` for the fleet-wide cap. Keep caps and
wait budgets positive; zero/invalid values fall back to their defaults. Lower
`LLM_REQUEST_TIMEOUT_MS` as a complement when slow in-flight calls need to be
aborted. Hard-stop semantics for `LLM_EXECUTION_ENABLED=false` are tracked in
...
- **Canonical safety matching (#501):** shared raw + canonical candidates cover input/history/tool scans and chat output prompt/vendor checks (de-accenting, `đ`, confusables, format/variation selectors, and `_`/`-`/`.` separators); credential-shape checks stay raw-only.
- **Harmful-output egress guard (#1377):** the shared final-output pipeline applies bounded Vietnamese/English/Chinese checks for actionable self-harm instructions/encouragement and targeted harassment after grounding, sanitization, and existing leak checks. A match replaces the complete reply with the generic blocked message, sets `skipHistory`, emits one redacted `HARMFUL_OUTPUT_BLOCKED` event, and never retries; prevention, academic, translation, fictional, and constructive-criticism controls remain allowed across Messenger, Discord, and Zalo.
- **Input classifier tier (#649, #1186, Messenger only, shadow-first)** — behind the regex guardrails, `PlatformAgentService` optionally runs `LlmContentClassifier` (`@wispace/chat-agent`) on the fresh user message (`redactSecrets` + `LLM_INPUT_CLASSIFIER_MAX_INPUT_CHARS` code points after redaction). Tier-1 single-turn, joint-scan, and length guards are authoritative preflight checks: a hit skips the classifier provider call and cannot be downgraded to `SAFE`; the classifier prompt treats learner text as untrusted data, so instructions inside it to control the label, confidence, reason, JSON verdict, or declare `SAFE` are `INJECTION` (#1378). Inputs within the ceiling are sent in full; longer inputs use one deterministic head-and-tail sample with an omission marker, single-turn, no history. The cheap-model `generateJson` call keeps its own `AbortSignal.timeout` deadline, no retry, no shared concurrency budget, and local circuit breaker (5 fails → open 30s → one half-open probe). Distress expressions remain eligible so crisis disclosures are not dropped. In shadow mode a typed classifier failure records bounded telemetry and continues the normal path; in enforce mode it returns the deterministic `CHAT_FAILURE_FALLBACK_MESSAGE` with `skipHistory=true` before the main LLM. A real `CRISIS` verdict in enforce mode uses the crisis handoff without the normal confidence floor; an outage never fabricates `CRISIS`. Usage is recorded under `LLM_INPUT_CLASSIFIER`, while safety events keep the existing redacted-excerpt-and-hash rule. `LLM_INPUT_CLASSIFIER_ENABLED` meters verdicts (`recordClassifierVerdict` → `llm_safety_events` `event_type='CLASSIFIER_FLAGGED'`, and `<prefix>_llm_classifier_verdict_total{label,mode,platform}`) plus input shape (`<prefix>_llm_classifier_input_total{shape,platform}`); `LLM_INPUT_CLASSIFIER_ENFORCE` additionally swaps the reply (`CRISIS` → crisis handoff; `ABUSE` → hostility deflection; `INJECTION` → prompt-injection block; extraction-flavoured `INJECTION` + `DISCLOSURE_PROBE` → non-disclosure line). Skip conditions (greeting / self-intro / off-topic / clarification choice) are re-checked in `runInputClassifier`. Label accuracy is tracked by the `classifier-eval-nightly` workflow (`npm run classifier:eval` in `@wispace/llm-agent`), **not** PR CI (#505); it fails on any `must-block` fixture miss while retaining the overall floor for adversarial probes. See `.claude/rules/prompts.md "Non-disclosure guard (#625)"`, ADR 0026 and ADR 0030.
...
- **Per-turn tool budget (#962):** tool executions accumulate across rounds — `maxToolExecutionsPerTurn` (default 8) bounds a single turn regardless of how calls distribute over the ≤6 rounds × ≤4 unique calls; the ceiling was 24 executions (≈48 upstream WISPACE requests) before this. Calls beyond the remaining allowance are refused as tool results (Vietnamese refusal the model can see) and the turn ends with the exhaustion partial answer when nothing remains — a learner-facing reply, never silent truncation. **Varied-argument loop detection:** one tool running ≥ `maxToolRunsPerNamePerTurn` (default 3) times per turn is cut off as a loop even when every round has different arguments (`pastDays:30` → `pastDays:31` defeats identical-signature matching); distinct same-tool lookups inside one round are unaffected. Read tools are deliberately budgeted per-turn (not per-day like write tools #626) — a per-turn ceiling bounds the worst case a confused or adversarial turn can drive, while read reuse across days is harmless. **Per-quota-unit ceiling for #791:** ≤ 8 executions × 2 upstream requests (`get_learning_progress_report` worst case) = **16 WISPACE requests per quota unit**; the fleet multiplier #791 builds on is (learners × turns × ≤16), asserted by test in `agent.service.spec.ts`.
- In-run tool observations (#414) are projected to server-derived fields, sanitized, bounded per result and cumulatively per round (including serialized tool arguments), and marked as `truncated`, `reused`, `dropped`, or `fallback` when data is reduced. **Stale observations (#1236)** are downgraded to a compact age marker before the next provider request when they reach `OPENAI_STALE_OBSERVATION_ROUNDS` (default 2); failed observations stay full for their first three tool rounds, while role/tool-call pairing and the existing hard-drop fallback remain unchanged. Every provider tool call still receives a paired tool message; observation metrics are `<prefix>_llm_observation_outcome_total{tool_name,platform,outcome}`.
- External data from WISPACE/user profile entering reminders/reports must be sanitized via `packages/llm-agent/src/safety/prompt-injection.utils.ts`.
- Generic JSON-object and required-string output from OpenAI is parsed via `@wispace/llm-agent/core`; Messenger supplies its text sanitizer, and reminder task-array validation stays in the Messenger formatter. Invalid shape falls back to a template; do not cast model output directly for formatting.
...
- **Shared utils** — `packages/bot-common/src/abort.utils.ts`: `isAbortError` (matches `AbortError` + `TimeoutError` deadlines) and signal-aware `sleep(ms, signal)` (rejects on abort). Re-exported by `packages/llm-agent/src/retry.utils.ts` and `packages/wispace-client/src/utils/with-retry.ts`.
- **LLM** — `LlmAgentInput.signal` / `LlmJsonRequest.signal` propagate through `agent.service` → OpenAI adapter (`completions.create` second arg) → failover loop stops on abort. The agent loop aborts the in-flight provider call on its own `globalAgentTimeoutMs` deadline; retry backoff sleeps abort when the signal fires. `LlmExecutionService` (Messenger) and `createEnvLlmExecutionPort` (shared port, Discord/Zalo chat + reports) accept `meta.signal` and pass the caller signal **composed with the per-call deadline into the provider request itself** (`fn(signal)`) — a deadline/cancellation aborts the underlying HTTP call, never just the wrapper promise. Caller cancellation and the global execution deadline stop retries; an internal per-attempt timeout is passed to the provider classifier so its existing retry policy decides. Study-reminder LLM calls carry the execution deadline; chat callers can pass `signal` end-to-end (chat processor → `PlatformAgentInput.signal` → agent loop).
...
- **Breaker accounting (#517)** — circuit breakers only count upstream-health failures, never deterministic per-user 4xx: the fleet-wide Meta Send API breaker (`MessengerOutboundService`, opossum `errorFilter` via `shouldTripMessengerBreaker` / `classifyMessengerSendApiError`) trips only on transport, 5xx, 408 (timeout/ambiguous), and 429 (Meta rate limiting); expired-24h-window and token errors stay per-user and never lock out healthy recipients. The generic `createCircuitBreaker` in `wispace-client` excludes non-5xx `WispaceApiError` + aborts by default. Metrics: `<prefix>_send_api_circuit_breaker_state`, `<prefix>_send_api_circuit_failures_total{error_class}`, `<prefix>_send_api_circuit_events_total{action,cause}`.
...
| Execution-port LLM retry — `retryWithBackoff` (`packages/llm-agent`, used by `createEnvLlmExecutionPort` + Messenger `LlmExecutionService`) | `min(baseDelayMs * 2^(attempt-1), LLM_OPENAI_RETRY_MAX_DELAY_MS)` via `cappedExponentialBackoff`, `maxAttempts` default 1 (budget: 1 main × 2 failover × N providers = 2N ≤ 8) | ✅ jittered after the cap; `rng` seam; per-attempt timeout via `LLM_RETRY_PER_ATTEMPT_TIMEOUT_MS` (default 10s) |
...
- **One operation has one budget:** admission, Redis-global concurrency, provider retries, failover, and the caller deadline share one `AbortSignal` and one total timeout while execution is enabled. `LLM_EXECUTION_ENABLED=false` bypasses that budget by invoking the callback directly; it is not an incident load-shed or hard-stop control. Use the bounded admission settings to reduce work, as described above.
...
- **Meta:** `PAGE_ACCESS_TOKEN`, `VERIFY_TOKEN`, `MESSENGER_APP_SECRET`, `MESSENGER_WEBHOOK_SIGNATURE_VERIFY`, `MESSENGER_PAGE_ID`, `GRAPH_API_VERSION`, `MESSENGER_PLATFORM_HEALTH_CHECK_INTERVAL_MS` (default `300000`), `MESSENGER_PLATFORM_HEALTH_STALE_MS` (default `300000`)
- **Platform readiness:** `DISCORD_READINESS_RECONNECT_GRACE_MS` (default `60000`); Zalo readiness follows `ZALO_TOKEN_REFRESH_CRON` and the existing 10-minute access-token safety buffer. `/health/ready` reads the local lifecycle snapshot and never calls Discord, Meta, or Zalo during a probe.
- **OpenAI (shared):** `OPENAI_API_KEY`, `OPENAI_MODEL` (the model is required for every active provider; no implicit fallback)
...
- **LLM provider policy (shared, required when a provider is active):** `LLM_ALLOWED_BASE_URLS` (CSV of exact, case-insensitive endpoint hostnames; vendor defaults count), `LLM_ALLOWED_MODELS` (CSV of exact, case-sensitive `provider:model` pairs). Both lists must be non-empty. Messenger keeps `LLM_API_KEY`, `LLM_MODEL`, `LLM_PROVIDER`, and `LLM_BASE_URL` as compatibility aliases; `LLM_BASE_URL` is Messenger-only and cannot conflict with `OPENAI_BASE_URL`.
- **LLM execution controls:** `LLM_EXECUTION_ENABLED`, `LLM_MAX_CONCURRENT`, `LLM_MAX_QUEUE_DEPTH`, `LLM_ADMISSION_WAIT_MS`, `LLM_BACKGROUND_ADMISSION_WAIT_MS`, `LLM_OPENAI_RETRY_MAX_ATTEMPTS` (default `1` — local per-layer attempt cap), `LLM_MAX_TOTAL_PROVIDER_ATTEMPTS` (default `6`, explicit `1..8` — shared actual provider-call cap per generation across chat tool rounds, retries, and failover), `LLM_OPENAI_RETRY_BACKOFF_MS`, `LLM_OPENAI_RETRY_MAX_DELAY_MS` (default `10000` — pre-jitter backoff ceiling, #577), `LLM_REQUEST_TIMEOUT_MS`, `LLM_RETRY_PER_ATTEMPT_TIMEOUT_MS` (default `10000` — per-provider call cap, #514). `LLM_EXECUTION_ENABLED=false` is a compatibility passthrough: normal bot wiring uses an unconfigured adapter and the deterministic fallback, but the execution wrapper still invokes callbacks directly without admission, deadline, retry, circuit protection, or the shared attempt budget, and active-provider policy validation is skipped. It is not a load-shed or hard-stop switch; keep it `true` and lower the admission caps/wait budgets above when reducing load. Hard-stop semantics are tracked in [#951](https://github.com/lengocanh2005it/wispace-bot/issues/951).
...
- **LLM usage (C2):** `LLM_USAGE_*`; single-provider USD estimate uses `LLM_COST_USD_PER_1M_INPUT_TOKENS_<MODEL>` / `LLM_COST_USD_PER_1M_OUTPUT_TOKENS_<MODEL>` (e.g. `gpt-5.4` → `GPT_5_4`). A failover chain requires provider-aware keys `LLM_COST_USD_PER_1M_INPUT_TOKENS_<PROVIDER>_<MODEL>` / `LLM_COST_USD_PER_1M_OUTPUT_TOKENS_<PROVIDER>_<MODEL>`; missing rates remain unpriced. Rates are estimates, not an invoice (OpenAI reference: [pricing](https://developers.openai.com/api/docs/pricing)).
- **LLM safety:** `LLM_SAFETY_EVENTS_ENABLED`, `LLM_SAFETY_WARNING_DAILY_THRESHOLD`, `LLM_SAFETY_EVENT_RETENTION_DAYS`
- **LLM input classifier (#649/#1186, Messenger only):** `LLM_INPUT_CLASSIFIER_ENABLED` (run + meter, default `false`), `LLM_INPUT_CLASSIFIER_ENFORCE` (swap reply on non-SAFE, default `false`, no effect unless enabled), `LLM_INPUT_CLASSIFIER_MODEL` (required when enabled and must be approved for the single active provider), `LLM_INPUT_CLASSIFIER_MAX_INPUT_CHARS` (maximum redacted Unicode code points sent to the classifier, default `512`; over-ceiling inputs use one head-and-tail sample), `LLM_INPUT_CLASSIFIER_TIMEOUT_MS` (default `1200`), `LLM_INPUT_CLASSIFIER_MIN_CONFIDENCE` (default `0`). An enabled classifier with multiple failover providers fails startup until feature routing (#417) exists; classifier usage is attributed to the `LLM_INPUT_CLASSIFIER` feature, and the nightly quality lane fails closed on any `must-block` fixture regression — see §7 "Input classifier tier", ADR 0030 and `apps/messenger-bot/.env.example`
- **WISPACE API (shared):** `WISPACE_API_USER_CALENDAR_URL`, `WISPACE_API_USER_GOALS_URL`, `WISPACE_API_TASK_SCORE_URL`, `WISPACE_API_PRECREATE_EXERCISE_URL`, `WISPACE_API_PRECREATE_EXERCISE_TIMEOUT_MS`, `WISPACE_API_LINK_STATUS_URL`, `WISPACE_LINK_STATUS_ENABLED`, `WISPACE_API_LINK_STATUS_TIMEOUT_MS`, `WISPACE_API_MAX_RETRIES`, `WISPACE_API_RETRY_BASE_DELAY_MS`, `WISPACE_HTTP_POOL_SIZE` (keep-alive connections per host, default 6 — #567), `WISPACE_INTERNAL_KEY` — auth: platform header (`x-psid`, `x-discordid`, or `x-zaloid`) + `X-Internal-Key`; next-exercise POST is idempotent and has no automatic retry. `WISPACE_API_LINK_STATUS_URL` returns `{status: "active", userId, ownershipVersion}` or `{status: "revoked", reason}`; HTTP 404/410 is authoritative revocation, while timeout/network/5xx is temporarily unknown. **Re-engagement client (#852):** `WISPACE_API_REENGAGEMENT_URL` (base URL, e.g. `https://backend.aihubproduction.com/api/bot/reengagement`; required by `createWispaceProviders` — every bot env needs it) serves `GET /candidates`, `GET /payload/{userId}`, `POST /mark-sent` with `X-Internal-Key` only (bot-level, no platform identity header); GETs retry on 5xx, `mark-sent` never retries (suppression log is single-shot). **Re-engagement send path (#853):** `POST /v1/discord/reengagement/run-once` (ops-guarded, body `{ userId }`) runs mapping lookup → payload → proactive embed+components DM (`sendProactivePayload`) → mark-sent for one learner — the dormancy gate (#595) deliberately does not apply (a dormant learner is a valid recipient) and ambiguous sends mark SUCCESS (anti-duplicate). **Batch cron (#854):** `discord-reengagement-cron` — daily scan gated by `REENGAGEMENT_ENABLED` (default off) with `REENGAGEMENT_CRON`/`REENGAGEMENT_TIMEZONE` (default 09:00 ICT), advisory lock `DISCORD_REENGAGEMENT` (884_200_950, contention logs+skips), single `getCandidates` page (`REENGAGEMENT_DAYS`=11, `REENGAGEMENT_LIMIT`=50, ceiling `REENGAGEMENT_MAX_PER_BATCH`=100, no upstream cursor), **bot-side #596 consent filter** (`NotificationPreferenceService.findReportOptedInUserIds` — the preference table lives in the bot DB so the backend cannot filter it), sequential sends paced by `REENGAGEMENT_SEND_GAP_MS` (500ms), `REENGAGEMENT_DRY_RUN` skips DM+mark-sent, one failure never aborts the batch, per-batch summary log with masked ids; the backend suppression rule is the only dedupe authority (no local sent-state). Counter `discord_reengagement_send_total{outcome=sent|ambiguous|rate_limited|failed|mark_sent_error|fetched|skipped}`. **Every upstream URL is validated fail-closed at startup** (`packages/wispace-client`): HTTPS required (dev-only `http://localhost` exception when `NODE_ENV != production`), no embedded credentials/fragments, no localhost/private targets in production, optional `WISPACE_ALLOWED_HOSTS` allowlist — a violation fails the app instead of sending the internal key/link tokens elsewhere. In production the link-status URL is required and reconciliation cannot be disabled. Unknown preserves the mapping and queued work for bounded retry; confirmed revocation cancels side effects and bumps the mapping generation. Local privacy unlink leaves a locally-unlinked mapping tombstone plus a hash-only audit event so stale verify intents cannot resurrect the mapping. Redacted transition audit rows are retained by `PLATFORM_LINK_AUDIT_RETENTION_DAYS` (default 90 days) and purged daily under a shared advisory lock; set `PLATFORM_LINK_AUDIT_CLEANUP_ENABLED=false` only during an incident.
...
- **Outbound learner-message backstop (#622):** `OUTBOUND_RATE_LIMIT_ENABLED` (default `true`), `OUTBOUND_RATE_LIMIT_MAX_MESSAGES` (default `30`), `OUTBOUND_RATE_LIMIT_WINDOW_MS` (default `600000`) — shared rolling cap across Messenger/Discord/Zalo; linked users share a bucket by WISPACE `userId`, otherwise `platform + external id`. Production requires Redis when enabled. A breach is a terminal `rate_limited` delivery outcome (chat refunds inbound quota; reminders/reports/dead-letter replay do not retry). Monitor `<prefix>_outbound_rate_limit_decisions_total{platform,outcome}` and use [the outbound rate-limit runbook](outbound-rate-limit.md) for first triage.
...
- **In-chat privacy confirm (#660, Messenger only):** `PRIVACY_CONFIRM_TTL_MS` (default `1800000` = 30 min) — how long a pending explicit unlink/delete/export confirmation prompt stays valid before it expires and a reply falls back to the normal chat pipeline. In-memory, pod-local; see §1.4 and #542 for durable persistence.
...
- **Secrets:** local development reads `.env`; production receives only the Vault AppRole bootstrap and resolves runtime secrets inside each container.
- **Vault runtime bootstrap (#653/#654/#655):** application startup loads string-valued secrets from the canonical shared and per-bot KV v2 paths described in [the Vault runbook](vault-secrets.md). Production uses `prd`, requires Vault, and receives only a mode-600 bootstrap file with per-bot AppRole credentials.
...
- **Redis (provisioned in production; `REDIS_ENABLED` flag retained):** `REDIS_ENABLED`, `REDIS_HOST`, `REDIS_PORT`, `REDIS_PASSWORD`, `REDIS_TLS`, `REDIS_CA`, `REDIS_PRIVATE_NETWORK`, `REDIS_COMMAND_TIMEOUT_MS` (default `2000`), `REDIS_CONNECT_TIMEOUT_MS` (default `5000`) — operation deadlines are validated as integers from `1` through `60000`; the native socket deadline is command timeout + `100ms`, disconnected commands reject instead of entering an offline queue, and unfulfilled commands are not resent after reconnect; R0–R4 stores + R5 user display cache; readiness via `GET /health/ready` when enabled
...
For production, follow the [Vault bootstrap runbook](vault-secrets.md); local development uses the app `.env` files.
...
GitHub Actions (push to `main`): [`.github/workflows/deploy-bots.yml`](../.github/workflows/deploy-bots.yml) (1 file, 3 jobs: messenger/discord/zalo → `deploy-bot-reusable.yml`) **builds + pushes images to GHCR and records the per-SHA CI gate** — it no longer SSHes into the VPS for normal deploys. Shared image build: [`deploy/Dockerfile.bot`](../deploy/Dockerfile.bot) (`ARG APP_NAME`).
...
**VPS self-pull instead:** a cron job on the VPS runs [`.github/scripts/vps-self-pull-deploy.sh`](../.github/scripts/vps-self-pull-deploy.sh) every few minutes — `git fetch`/`reset` a local clone, verify the exact `HEAD` SHA against the required GitHub Actions jobs, check GHCR for an image tagged with that SHA, and if all gates pass, run the existing [`vps-deploy.sh`](../.github/scripts/vps-deploy.sh) (unchanged: blue-green swap, health check, migrations, nginx switch). All outbound from the VPS, so the inbound edge-filter never applies. One-time setup (git clone + crontab entry + `GHCR_USER`/`GHCR_PULL_TOKEN` + separate read-only `GITHUB_API_READ_TOKEN`) is documented in the script's header comment.
...
**CI gate (#694):** `deploy-bots.yml` now runs for every push to `main`; its reusable `changes` jobs still skip image builds for docs/spec-only commits. Before any GHCR lookup or fallback target resolution, self-pull queries the Actions API with the separate read-only `GITHUB_API_READ_TOKEN` and requires the exact `push` run for `HEAD`: `Verify Pull Request / verify`, then each app's `changes` + `build-image` + `runtime-image-check`, with Messenger additionally requiring `migration-timestamps` and `migrations-check`. A pending, missing, malformed, rate-limited, or transient API result waits up to `CI_GATE_WAIT_TIMEOUT_SECONDS` (default 30 minutes), then uses the existing `vps_self_pull_stall` alert; a 401/403 authentication error, failed global job, or successful build with a missing GHCR image fails closed immediately. Messenger CI failures block the fleet; Discord/Zalo runtime failures mark only that app and leave the other app deploys eligible. A deliberately skipped build is the only expected no-image case; an image is never silently treated as absent after a successful build. Monitoring expiry/revocation of `GITHUB_API_READ_TOKEN` is intentionally outside #694 and must be added to the credential-monitoring follow-up (#604 or a dedicated issue); until then, a revoked token fails closed and alerts through `vps_self_pull_stall`.

**Deploy hardening (#199/#201/#203/#204/#275/#276/#278/#654/#655):** `vps-deploy.sh` fails closed before cutover when the Vault bootstrap is missing/invalid, the migration writer endpoint is unreachable/not writable, or the nginx upstream conf is missing (`SKIP_NGINX_CHECK=true` is the first-deploy escape hatch). The pre-cutover health gate allows a 4-minute cold start by default (`HEALTH_MAX_ATTEMPTS=120`, 2s interval); operators can override it for slower hosts. After `migration:run`, the new immutable release image runs `migration:show`; any pending or unverifiable migration status blocks nginx cutover. The live container is detected by the port nginx currently routes to (never removed by name); an interrupted deploy that left nginx routed to `${APP}-new` adopts it as `${APP}-old` instead of deleting it. Container start/stop honor `DOCKER_STOP_TIMEOUT` (default 60s) for the 45s app drain window. Migration fencing is implemented by the TypeORM data source on a dedicated advisory-lock session; the deploy script invokes the validated Vault runner directly, so a shell escape cannot release the fence early. Post-switch monitoring verifies the **public nginx route** (`curl --resolve …:443:127.0.0.1 https://aiassist.aihubproduction.com/health[/discord|/zalo]/ready`) instead of only the standby port. Runtime images include `postgresql-client` (psql) for endpoint preflight and dumps. Uploads exclude `.env` from `rsync --delete`; only the mode-600 Vault bootstrap is delivered. `vps-deploy.sh` atomically replaces the app `.env` from that allowlisted bootstrap and never creates a full secret-bearing runtime env file. `postgres-backup.sh` enforces `umask 077` + mode 700/600. **Bootstrap rollback (#932):** the deploy proves the bootstrap AppRole by logging in against Vault (`/v1/auth/approle/login`, probe token revoked) **before** the live `.env` is touched — a wrong, rotated, or expired `VAULT_SECRET_ID` refuses the deploy instead of arming the host. The probe consumes one AppRole `secret_id` use per deploy (two when the first login response is lost and `--retry` re-POSTs), so keep `secret_id_num_uses` ≥ 3. A mode-600 `.env.pre-deploy` snapshot is taken before the atomic install; the EXIT trap restores it byte-identically on any failed or aborted exit after the install (a first deploy removes the installed `.env` instead) and logs the restore, so a rolled-back deploy no longer leaves a bad bootstrap in place for the next container restart. A successful deploy drops the snapshot. `sync-env.yml` fans out per-app jobs, so each bot's snapshot/restore is independent. **Metrics recovery (#278):** all bare-run blue-green containers join the external Docker network `monitoring`; Prometheus scrapes stable `<app>-metrics` aliases on fixed internal ports (`messenger=5007`, `discord=3001`, `zalo=3002`), while host active/standby ports remain nginx's cutover mechanism. The deploy script verifies protected `/metrics` returns 401/403 on the standby port and through the monitoring-network container IP after alias handoff; alias/network failures roll back nginx before the old container is stopped.

**Migration identity compatibility (#340):** every runtime migration class and effective TypeORM name has a unique 13-digit timestamp. The four historical names that collided (`1751029200016`/`1751029200017`) are mapped to their corrected names by `guardDataSourceMigrations`: fresh databases run the corrected migrations, while existing rows remain untouched and are treated as applied; legacy rows can also be reverted through the corrected implementation. `.github/scripts/check-migration-timestamps.sh` checks filename, class, and effective-name timestamps before the migration barrier.
...
- **Cause attribution.** A failed `docker login` fails closed before CI/image fallback, so an expired pull token (#604) cannot be mistaken for a missing build. A no-image commit is considered expected only when the exact CI build job is `skipped`; a successful build with no manifest is a gate failure.
...
| `GITHUB_API_READ_TOKEN` (VPS host)                     | Separate fine-grained, read-only Actions/Checks token used by VPS self-pull to read exact-SHA workflow/job status (never use the GHCR token)                  |
...
Production setup and recovery: [vault-secrets.md](vault-secrets.md).
...
1. **Rotate immediately** — every credential in that file: DB password, `INTERNAL_API_KEY`, `WISPACE_INTERNAL_KEY`, Meta/Discord/Zalo tokens and secrets, OpenAI/LLM provider keys, and Vault AppRole credentials. Treat them as compromised regardless of git status (git-ignore prevents commits, not leaks). This includes `/home/ngoc_anh/<app>/.env` or any downloaded bootstrap artifact if a production artifact was exposed.
...
3. **Update Vault** — write the rotated values to the canonical `prd` KV paths, rotate the affected AppRole credentials in GitHub, and run the manual **Sync production env** workflow so deployed bots pick them up. The deploy scripts only install the bootstrap; they never rotate secrets automatically.
