...
- **Input classifier tier (#649, Messenger only, shadow-first)** — behind the regex guardrails, `PlatformAgentService` optionally runs `LlmContentClassifier` (`@wispace/chat-agent`) on the fresh user message (`redactSecrets` + truncate 512, single-turn, no history), classifying `SAFE | INJECTION | DISCLOSURE_PROBE` via a cheap-model `generateJson` call on its own path — own `AbortSignal.timeout` deadline, no retry, no shared concurrency budget, local circuit breaker (5 fails → open 30s → one half-open probe). Skipped for distress messages. Fail-open on every failure. `LLM_INPUT_CLASSIFIER_ENABLED` meters every verdict (`recordClassifierVerdict` → `llm_safety_events` `event_type='CLASSIFIER_FLAGGED'`, and `<prefix>_llm_classifier_verdict_total{label,mode,platform}`); `LLM_INPUT_CLASSIFIER_ENFORCE` additionally swaps the reply (`INJECTION` → prompt-injection block; extraction-flavoured `INJECTION` + `DISCLOSURE_PROBE` → non-disclosure line). Skip conditions (distress / greeting / self-intro / off-topic / clarification choice) are re-checked in `runInputClassifier`. Label accuracy is tracked by the `classifier-eval-nightly` workflow (`npm run classifier:eval` in `@wispace/llm-agent`), **not** PR CI (#505). See `.claude/rules/prompts.md` "Non-disclosure guard (#625)".
...
- **LLM input classifier (#649, Messenger only):** `LLM_INPUT_CLASSIFIER_ENABLED` (run + meter, default `false`), `LLM_INPUT_CLASSIFIER_ENFORCE` (swap reply on non-SAFE, default `false`, no effect unless enabled), `LLM_INPUT_CLASSIFIER_MODEL` (required when enabled and must be approved for the single active provider), `LLM_INPUT_CLASSIFIER_TIMEOUT_MS` (default `1200`), `LLM_INPUT_CLASSIFIER_MIN_CONFIDENCE` (default `0`). An enabled classifier with multiple failover providers fails startup until feature routing (#417) exists — see §7 "Input classifier tier" and `apps/messenger-bot/.env.example`
