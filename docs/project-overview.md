...
- **Canonical safety matching (#501):** shared raw + canonical candidates cover input/history/tool scans and chat/compaction output prompt/vendor checks (de-accenting, `đ`, confusables, format/variation selectors, and `_`/`-`/`.` separators); credential-shape checks stay raw-only.
