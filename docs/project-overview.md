...
- **In-chat privacy confirm (#660, Messenger only):** `MessengerChatProcessorService` intercepts privacy requests (unlink / delete / export keyword → `detectPrivacyIntent`) **before** the quota block, so the prompt costs no free-form slot and never reaches the LLM. While a pending action exists (`PrivacyStateService`, in-memory, TTL `PRIVACY_CONFIRM_TTL_MS`, default 30 min), **every** message routes to the privacy handler: a bare `Có`/synonym executes it, a bare `Không`/synonym cancels it, anything else re-sends the `Có/Không` reminder — none of these consume quota or an LLM round. The inbound consent/cancel is written to `message_logs` (`PRIVACY_CONFIRM_IN` / `PRIVACY_CANCEL_IN`) before the irreversible step. After TTL expiry a reply falls through to the normal pipeline. Durable / cross-pod persistence of the pending state is #542.
...
- **In-chat privacy confirm (#660, Messenger only):** `PRIVACY_CONFIRM_TTL_MS` (default `1800000` = 30 min) — how long a pending unlink/delete/export `Có/Không` prompt stays valid before it expires and a reply falls back to the normal chat pipeline. In-memory, pod-local; see §1.4 and #542 for durable persistence.
