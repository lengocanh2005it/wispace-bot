...
All `chat.completions.create` calls go through **`LlmExecutionService`** (`src/modules/llm-execution/`) — caps concurrent in-process calls (`p-limit`, `LLM_MAX_CONCURRENT`), retries 429/5xx (`LLM_OPENAI_RETRY_*`), and enforces a per-request deadline (`LLM_REQUEST_TIMEOUT_MS`) that **aborts the in-flight request** (composed with the caller's AbortSignal), not just the wrapper promise. Disable gate: `LLM_EXECUTION_ENABLED=false`. **Free-form chat on all three bots uses the same contract** — Messenger chat injects `LlmExecutionService` into `PlatformAgentService`; Discord/Zalo build the shared env-driven port (`createEnvLlmExecutionPort` in `packages/llm-agent`) with identical env keys. Scaling ≥2 pods: set `LLM_GLOBAL_CONCURRENCY_ENABLED=true` (+ `REDIS_ENABLED`) for one Redis-distributed aggregate budget (key `llm:concurrency:global`) shared across all pods and bots.
...
- **LLM execution gate:** `LLM_EXECUTION_ENABLED`, `LLM_MAX_CONCURRENT`, `LLM_OPENAI_RETRY_MAX_ATTEMPTS`, `LLM_OPENAI_RETRY_BACKOFF_MS`, `LLM_REQUEST_TIMEOUT_MS`
...
`GET /v1/discord/oauth/callback` → exchange code → verify WISPACE token → **persist verify intent** (`discord_link_verify_records`) → **`upsertLink` immediately** (retried, since WISPACE already consumed the single-use token) → consume intent (fire-and-forget) → in guild? send welcome DM + `markWelcomed` : redirect straight to `DISCORD_INVITE_URL`. Joining the server is only needed to *receive* the welcome DM — the bot re-sends it on `guildMemberAdd` for already-linked users (`findUserIdByDiscordId`) **only when not welcomed within `DISCORD_REWELCOME_WINDOW_MS`** (default 24h, `last_welcomed_at` marker dedupes re-joins and the join-during-callback race). A crash between verify and upsert is reconciled by the `discord-link-reconcile` cron (5 min, advisory lock `DISCORD_LINK_RECONCILE`) — re-commits the mapping from the stored `userId`; records older than `DISCORD_LINK_RECONCILE_MAX_AGE_MS` with no mapping are dropped with an error log (user retries with a fresh token). Relinking the same Discord ID to a different WISPACE user sends a DM notice to the account and logs a warning — the displaced user silently loses the link (by design). DM delivery failures (e.g. privacy-blocked users) increment `discord_dm_delivery_failures_total{reason}` (Prometheus). The frontend portal has no callback page and needs nothing from the redirect; WISPACE marks the link itself at verify time, which now matches the bot's mapping exactly. The portal distinguishes "linked + not joined" via `GET /v1/discord/link-status?userId=` (ops-guarded, returns `{ linked, inGuild }`) to show the join hint only when relevant.
