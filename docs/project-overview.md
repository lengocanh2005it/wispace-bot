...
- **Input classifier tier (#649, #1186, Messenger only, shadow-first)** — behind the regex guardrails, `PlatformAgentService` optionally runs `LlmContentClassifier` (`@wispace/chat-agent`) on the fresh user message (`redactSecrets` + `LLM_INPUT_CLASSIFIER_MAX_INPUT_CHARS` code points after redaction). Inputs within the ceiling are sent in full; longer inputs use one deterministic head-and-tail sample with an omission marker, single-turn, no history. The cheap-model `generateJson` call keeps its own `AbortSignal.timeout` deadline, no retry, no shared concurrency budget, and local circuit breaker (5 fails → open 30s → one half-open probe). Skipped for distress messages. Fail-open on every failure. `LLM_INPUT_CLASSIFIER_ENABLED` meters every verdict (`recordClassifierVerdict` → `llm_safety_events` `event_type='CLASSIFIER_FLAGGED'`, and `<prefix>_llm_classifier_verdict_total{label,mode,platform}`) plus input shape (`<prefix>_llm_classifier_input_total{shape,platform}`); `LLM_INPUT_CLASSIFIER_ENFORCE` additionally swaps the reply (`INJECTION` → prompt-injection block; extraction-flavoured `INJECTION` + `DISCLOSURE_PROBE` → non-disclosure line). Skip conditions (distress / greeting / self-intro / off-topic / clarification choice) are re-checked in `runInputClassifier`. Label accuracy is tracked by the `classifier-eval-nightly` workflow (`npm run classifier:eval` in `@wispace/llm-agent`), **not** PR CI (#505). See `.claude/rules/prompts.md` "Non-disclosure guard (#625)" and ADR 0026.
