...
- Updated `Dockerfile`, `.github/workflows/deploy-messenger-bot.yml` (path filter + `turbo run ... --filter=@wispace/messenger-bot...`).
...
## Phase 3 — Implement `apps/discord-bot` (chat + quota/usage/safety + account-linking + 7/7 real tool handlers COMPLETED; register_exam_report_notifications NOT NEEDED — no 24h limit)
...
- `DiscordAgentToolsService` — **historical MVP stub** at the time of this phase snapshot; the current implementation has 7/7 real WISPACE tool handlers (incl. `precreate_next_exercise`) plus OAuth account linking (see the current Phase 3 implementation below).
...
## Phase 4 — Implement `apps/zalo-bot` (COMPLETED)
...
## Phase 5 — Fully independent CI/CD per bot (COMPLETED)

Single `deploy-bots.yml` with 3 jobs (messenger/discord/zalo) + shared `deploy-bot-reusable.yml` + `deploy/Dockerfile.bot`. Each bot deploys independently via path filtering, with shared secrets and migration gate (messenger-bot only runs migrations).
