...
| **Vault runtime secret contract (#653)** | duplicated per-bot bootstrap loaders                 | Shared `@wispace/bot-common` KV v2 loader + canonical shared/per-bot paths — [vault-secrets.md](vault-secrets.md); production delivery #654 and Doppler removal #655 remain | Done (runtime contract) |
