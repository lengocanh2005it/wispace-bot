...
| **R7** ✓          | Batch canonical-platform lookup (#954)                     | 1 day                                                        | One set-based PostgreSQL lookup per report page across Messenger, Discord, and Zalo; no O(n) fallback                                                                                                                                                                                                              |
...
| **PRIVACY** ✓     | Durable, ownership-safe privacy erasure (#995)             | 2–3 days                                                     | Shared cleanup jobs, truthful 200/202/409 outcomes, own-platform reconcilers, relink fencing, metrics/Ops Health, and seven-day terminal retention; real fleet drill remains an operator verification step                                                                                                         |
...
| **migration-identities** (#340) | **Unique TypeORM migration timestamps** — the four historical duplicate names now use unique runtime names; `guardDataSourceMigrations` recognizes applied legacy rows without rewriting them and supports legacy rollback; CI checks filename, class, and effective-name collisions                                                                                                                                                                                                                                                                                                                                    |
...
- **MVCC / concurrency direction (#576/#705)** — `READ COMMITTED` is the baseline for every concurrent flow; no generic application-level MVCC layer. Per-flow mechanism (single-statement CAS, lease token, `FOR UPDATE`, one `version` CAS, advisory lock, Redis Lua) inventoried. The shared `@wispace/reschedule-confirm` mutation boundary now invalidates Discord/Zalo calendar caches after a committed reschedule and claim cleanup; read-your-writes and duplicate-confirm coverage lives in the shared service tests. See [ADR-0008](adr/0008-mvcc-and-optimistic-concurrency.md).
- **Privacy erasure recovery (#995/#1128–#1131)** — `privacy_cleanup_jobs` are created atomically with unlink/delete, request cleanup returns truthful complete/incomplete outcomes, and each bot reconciles only its own stores with a generation fence. See [ADR-0014](adr/0014-privacy-erasure-completion.md) and the [fleet verification drill](privacy-erasure-verification.md).

### Gaps & Remediation

| Gap                                            | Impact                                                                                                                                                 | Fix                                                                                                                                                                                                                                                                                                                                                                                             | Phase              |
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------ |
| Standalone chat intent matching (#941)         | Prefix matching swallowed questions after greeting or self-intro keywords                                                                              | **Done (#941)** — shared full-message matching keeps bare greetings/self-intros direct and routes content-bearing text through the normal Messenger/Discord/Zalo chat path; regression fixtures cover all three ingress paths                                                                                                                                                                   | **Done** (#941)    |
| Clarification choice replay (#1035)            | Consumed menu state treated any later choice text as a replay, so new "lịch học" / "tiến độ" / "đổi lịch" messages were silently swallowed for the TTL | **Done (#1035)** — consumed state suppresses only a known event identity; new or id-less events reach the normal agent, failed choice delivery is retryable, and `skip_delivery` has its own metric; Redis accepts live state across lowered runtime limits                                                                                                                                     | **Done** (#1035)   |
| Reschedule confirmation safety (#1037)         | Stop acknowledgements could leave a staged proposal armed, and Zalo bare affirmatives could authorize an unrelated calendar change                     | **Done (#1037)** — stop/cancel clears pending platform-scoped proposals, approvals remain button/token-bound, lazy expiry is explicit, unrelated chat is preserved, and failed/ambiguous proposal delivery removes the staged request                                                                                                                                                           | **Done** (#1037)   |
| Abort-safe reschedule staging (#1062)          | A timed-out Discord/Zalo tool could finish staging after the caller stopped waiting and leave an invisible proposal or prompt                          | **Done (#1062)** — optional abort signals reach calendar/save, late cleanup is nonce-conditional, the confirmation boundary is enforced, and token-less shared stages fail closed                                                                                                                                                                                                               | **Done** (#1062)   |
...
| Canonical LLM safety matching (#501)           | Diacritics, separators, format characters, and confusables could bypass shared input or final-output checks                                            | **Done (#501)** — shared raw + canonical candidates in `@wispace/llm-agent`; centralized input/history/tool scanning; prompt/vendor output checks cover chat; credential shapes remain raw-only; separator/format regressions are must-block fixtures                                                                                                                              | **Done** (#501)    |
| Harmful model-output egress (#1377)            | Input checks could pass while a model reply contained actionable self-harm guidance or targeted harassment                                             | **Done (#1377)** — shared bounded VN/EN/zh final-output guard; grounding/leak precedence preserved; complete reply replacement + `skipHistory`; one redacted `HARMFUL_OUTPUT_BLOCKED` event; academic/prevention/fiction/translation/constructive controls are fixture-covered | **Done** (#1377) |
...
| Fail-closed LLM provider configuration (#1075) | Unsafe or drifted provider endpoints/models could pass startup and fail only on a learner request                                                      | **Done (#1075; #1104–#1106)** — shared exact-host URL + exact provider/model policy validates all active candidates before lazy SDK use; Messenger aliases and classifier/failover ambiguity fail closed                                                                                                                                                                                        | **Done**           |
...
| Learner-level cross-platform state (#637/#1177) | Linked channels could split quota, reminders, or daily-report claims; identity churn could reset chat usage                                                                 | **Done (#637; #1177)** — canonical numeric WISPACE `userId`, shared learner bucket, independent anonymous bucket, canonical reminder owner, atomic learner report claim, and unlink-vs-delete semantics. #1177 removes active-mapping adoption of anonymous quota: owner-aware `chat_daily_usage` uniqueness, charge-owner snapshots for refund/recovery/audit/rebuild, and an erasure path that never deletes another owner's row. Related: #512/#458/#510/#461 | **Done** |
| Messenger reminder opt-out evidence (#942)     | Production selection was fixed, but mocked tests did not prove SQL semantics or outbound suppression                                                   | **Done (#942/#955)** — real PostgreSQL smoke covers `false` excluded, `true`/`NULL` included, and the sync → dispatch path with no outbound attempt for opted-out learners                                                                                                                                                                                                                      | **Done**           |
...
| **≥2 reminder cron pods**                          | `claimJob` ✓ + **cron pg_advisory_lock** ✓           | Batch reminder upsert TOCTOU fixed ✓ (`pg_advisory_xact_lock`)                                                                                                                    | Done              |
...
| Metric alert coverage (#684)                       | 35 emitted metric families had no rule               | Platform-prefixed leading-indicator rules, cron success heartbeats, promtool/synthetic tests, and a metric classification/runbook — [`monitoring-alerts.md`](monitoring-alerts.md) | **Done (#684)**   |
...
| **LLM retry layers multiply (#1247)**              | Agent, execution, and failover counters could multiply provider calls | **Done (#1247)** — one shared 1..8 provider-attempt cap per generation (default 6), preserved abort/fallback semantics, bounded total-attempt telemetry, and fresh budget per queue replay | **Done** |
