...
> **Current status (2026-08-14):** Shared migrations now live in `packages/database/src/migrations/`; that directory contains 32 migration files, including the later Zalo, idempotency, cleanup, dead-letter, reschedule, leader-lease, durable-inbox, and Zalo OA-token-version migrations. The centralized `apps/messenger-bot` migration pipeline remains current. The 18-file ownership table below is intentionally preserved as the historical design baseline.
